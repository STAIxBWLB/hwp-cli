...
pub const KO: &[(&str, &str, &str)] = &[
    ...
    (
        "new",
        "template",
        "내장 문서 템플릿을 영문 슬러그 또는 한국어 별칭으로 사용 (--list-templates 참고). --from과는 함께 쓸 수 없음. 프레임 플래그와는 함께 쓸 수 있으며, 골격이 담지 않는 두문/결문 표를 플래그가 더한다",
    ),
    (
        "new",
        "list_templates",
    ...
    (
        "new",
        "doc_foot",
        "공문서 결문 블록 \"키=값\" (키: 발신명의|기안자|검토자|결재자|협조자|시행번호|시행일자|접수번호|접수일자|주소|홈페이지|전화|팩스|이메일|공개구분, 반복 가능)",
    ),
    (
        "new",
        "notice_head",
