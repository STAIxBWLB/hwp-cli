...
pub const KO: &[(&str, &str, &str)] = &[
    ...
    (
        "render",
        "layout_json",
        "쪽별 세그먼트 기하(hwp-render-layout-v1)를 원자적으로 기록. 상자는 쪽 좌상단을 원점으로 한 포인트 단위라 PNG·SVG·PDF와 모든 --dpi에서 동일한 파일이 나옴. 기록은 옵트인이며 비용은 비대칭: 이 플래그가 없으면 비용이 전혀 없고, 있으면 스팬 기록을 위해 문서를 한 번 더 배치하므로 렌더 시간이 대략 두 배가 됨",
    ),
    ...
    (
        "edit",
        "seal",
        "도장 날인 \"앵커=>경로\" 또는 \"앵커=>경로@크기mm\" — 앵커 문구 위에 도장 부유 배치. \
         도장은 글 앞에 떠서 겹치므로 배경이 불투명하면 아래 내용을 가린다 — 배경이 투명한 \
         이미지를 쓸 것 (반복 가능)",
    ),
    (
        "edit",
    ...
    (
        "edit",
        "insert_para",
        "문단 삽입 \"앵커=>텍스트\" — 앵커가 있는 문단 뒤에 새 문단. 앵커는 표 셀·중첩 표·캡션 안에서도 찾는다 (반복 가능)",
    ),
    (
        "edit",
        "insert_para_before",
        "문단 삽입(앞) \"앵커=>텍스트\" — 앵커가 있는 문단 앞에 새 문단. 앵커는 표 셀·중첩 표·캡션 안에서도 찾는다 (반복 가능)",
    ),
    (
        "edit",
        "delete_para",
        "문단 삭제 \"텍스트\" — 텍스트가 있는 문단 삭제. 표 셀·캡션 안도 지우며, 리스트마다 최소 1문단은 남긴다 (반복 가능)",
    ),
    (
        "edit",
        "add_row",
    ...
    (
        "slots",
        "forms",
        "한국 공문서 양식 필드도 나열 — 레이블 셀, 본문의 \"라벨: 값\", 자리표시자를 키·레이블·출처·신뢰도·등장 횟수·필수 여부와 함께 (JSON \"fields\"; kordoc 규칙)",
    ),
    // fill
    (
        "fill",
    ...
    (
        "fill",
        "set",
        "자리표시자 채우기 \"이름=값\" (반복 가능; `{{이름}}` 치환, 공백을 넣은 `{{ 이름 }}`도 포함). \"이름=@부분.md\"이면 `{{이름}}` 앵커 문단을 부분 파일(md+HTML 표 블록, 계약 docs/design/18)로 교체 — 대규모 문서의 부분별 조합. \"@@\"는 리터럴 '@'. 이름은 첫 '='에서 끝나므로 '='가 든 이름은 --data로 넘김",
    ),
    (
        "fill",
        "data",
        "자리표시자 이름→값 JSON 객체 파일 (일괄 채우기; 이름은 `{{ 이름 }}`처럼 앞뒤 공백 제거; \"parts\": {\"이름\": \"경로\"} 부분 파일 교체, \"tables\": [...] 표 행 채우기)",
    ),
    (
        "fill",
        "json",
    ...
    (
        "fill",
        "allow_partial",
        "일부 요청이 자리를 찾지 못해도 일치한 값만 게시 (기본: 하나라도 미치환이면 실패). 하나도 일치하지 않으면 입력을 그대로 게시하고 모든 건수를 0으로 보고",
    ),
    (
        "fill",
        "forms",
        "같은 값으로 한국 공문서 양식 필드도 채움 (hwpx) — 레이블 셀 옆·아래 칸, 본문의 \"라벨: 값\", \"라벨(  )\" 빈칸, \"□항목\" 확인란, \"(라벨:  )\" 빈칸. 비어 있지 않은 값 칸과 \"라벨:\" 뒤 본문 텍스트는 덮어씀(덮어쓸 때마다 경고), 누름틀·그림·표가 든 칸은 쓰지 않으며 값은 그대로 넣음. 키는 공백·쌍점·괄호를 무시하고 비교하며, 보고서에 \"unmatched\"가 추가됨. 출력은 .hwpx",
    ),
    // validate
    (
        "validate",
