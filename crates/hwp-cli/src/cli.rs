...
pub enum Cmd {
    ...
    Fill {
        ...
        allow_partial: bool,
        /// Also fill Korean form fields from the same values (hwpx): the cell next to or below a label cell, inline "label: value", "label(  )" blanks, "□option" checkboxes and "(label:  )" blanks. A non-empty value cell, and the inline text after "label:", is overwritten (a warning names each overwrite); a cell holding a field, picture or table is never written, and values are inserted literally. Keys match with spaces, colons and parentheses ignored; the report adds "unmatched". Output must be .hwpx
        #[arg(long)]
        forms: bool,
    },
