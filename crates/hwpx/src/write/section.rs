        ...
        match ch {
            ...
            HwpChar::ExtCtrl { ctrl_index, .. } => {
                ...
                match control {
                    ...
                    Control::Generic(g)
                        if !g.gso_shapes.is_empty()
                            && g.hwpx_raw_xml.is_none()
                            && g.container_box.is_none() =>
                    {
                        ...
                        // 원문이 지워진(edit 헬퍼 무효화) 컨테이너는 자식 좌표가 컨테이너
                        // 상대라 합성하면 위치가 틀어지므로 fail-closed로 둔다(아래
                        // OpaqueControlUnrepresentable typed loss로 떨어진다).
                        open_run!(cur_shape);
                        flush_text(out, &mut text_buf, &mut pending_tabs);
                        shape_break!();
                        let before = out.len();
...
fn write_picture(
    ...
    let angle = pic.rotation.unwrap_or(0.0).round() as i32;
    let brightness = pic.brightness.clamp(-100, 100);
    let contrast = pic.contrast.clamp(-100, 100);
    let [cl, ct, cr, cb] = pic
        .crop
