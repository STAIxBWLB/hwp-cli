...
def make_circle_png(size, rgb):
    """Return an RGBA seal rim with a transparent interior and exterior."""
    r, g, b = rgb
    cx = cy = (size - 1) / 2.0
    rad = size / 2.0
    raw = bytearray()
    for y in range(size):
        ...
        for x in range(size):
            inside = (rad - 2) ** 2 <= (x - cx) ** 2 + (y - cy) ** 2 <= rad * rad
...
# 이 파일은 정답지 유래이므로 산출물만 dest(~/Documents/hwp-verification)에 두고,
