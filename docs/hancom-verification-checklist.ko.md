...
실행을 실패시켜야 한다. 목록에 있는 케이스가 실패해도 게시되지 않고 통과로 세지도 않는다.
인덱스의 `known_failures` 배열에 하네스가 낸 실패 줄 그대로와, 그것을 추적하는 이슈, 제외를
허용한 변수 이름이 기록되며, 마감 보고는 제외된 케이스 수를 명시한다. 스크립트가 추적하지 않는
id는 산출물을 하나라도 만들기 전에 거부하고, 목록에 있는데 통과한 케이스는 정상적으로 게시하면서
"listed but passed"로 보고하므로 낡은 항목이 그것이 덮어 주던 결함보다 오래 남지 못한다. 모든
제외 항목은 릴리스 검증 블록(4단계 04-05 계획)으로 그대로 이어진다. **제외가 있는 실행은 깨끗한
통과가 아니며**, 해당 이슈들이 닫히고 변수를 다시 비울 때까지 마일스톤은 깨끗하지 않다.
