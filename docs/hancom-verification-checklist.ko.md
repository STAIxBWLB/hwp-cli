...
담으므로 산출물을 언제나 그것을 판정하는 표 항목으로 되짚을 수 있다.
...
이 실행은 Hancom 관찰도 영수증도 만들지 않는다. 대신 산출물 옆에 그 산출물에 결합될 영수증
하나를 지정한 `<artifact>.policy.json`과, 그 영수증이 놓일 빈 `receipts/` 디렉터리를 만든다.
실제 관찰이 생긴 뒤에는
`hwp certify <artifact> --policy <artifact>.policy.json --report <directory>` 한 명령으로
손수 고칠 것 없이 확인할 수 있다. 통과 영수증을 미리 채우지 않는다.
