...
| File | What it tests | How to check |
|---|---|---|
| `L1_수식.hwpx` | Three inline equations (a fraction plus a radical, a summation, XML special characters, and Hancom's own measured ampersand escape) | Three equations appear **side by side like characters** at the end of the "수식 자리:" paragraph: 1. a fraction and radical in the form `a/b + √(x²+y²)`, 2. a summation with limits above and below Σ, and 3. `x < y & y > z` where **the inequality signs display as-is and the ampersand displays as a bare literal `&` with no quote marks** (their absence indicates an esc-to-entity round-trip defect; a stray pair of quote marks around the `&` indicates Hancom's own quoted-literal escape stopped being honored). The stored script for equation 3 is `x < y "&" y > z`: Hancom's own equation grammar reserves a bare `&` as the matrix column-alignment token (it renders nothing outside a matrix), and the ASCII-double-quote form is the escape Hancom itself measured to display it as a literal character (07-hangul-compat-rules.md I1). Double-clicking an equation should open the equation editor with the original script |
