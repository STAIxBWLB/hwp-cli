...
- `hwp mcp` stdio를 기본 local transport로 유지하고 20개 tool 동작의 호환성을 보존.
...
1. **Core extraction:** typed JSON-RPC value와 authority context 도입. stdio output과 20개 tool의
...
4. **Quick Web pilot:** non-production connector 등록. 20개 tool의 remote-safe parity, upload,
...
- stdio가 기본값으로 남고 기존 process test가 정확히 20개 tool을 계속 노출;
...
- tenant-isolated test environment에서 Quick Web이 initialize, 의도한 20개 remote-safe tool list,
