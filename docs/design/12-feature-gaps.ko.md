...
| GG-12 | ~~**개요(outline) 번호 미렌더** — head_type 2(번호)·3(불릿)만 마커 생성, 1(개요)은 제외~~ | `hwp-model/src/list.rs`(`ListState::marker_for_render`) | ✅ **해소(2026-08-13)** — 개요(head_type 1) 문단에 수준별 고정 마커(`1.` / `가.` / `1)` / `가)` / `(1)` / `(가)` / `①`) 렌더. 개요 `numbering_id`는 비정규화 원시값이라 전용 카운터 계열 사용. 텍스트 변환은 `marker()`로 헤딩 매핑만 유지(이중 번호 방지) | M |
...
[정답지 확보]  ──선행──▶  GB-1~7(개체 렌더)  ──필요──▶  10/11 레코드 구조 해석
   ...
   │                       GG-7/GG-14~15/GG-18/19 ── 정품 렌더 픽셀 대조
   │
...
- **GG-7, GG-14~GG-15, GG-18, GG-19**: 정품 렌더와의 픽셀 대조로 확정
