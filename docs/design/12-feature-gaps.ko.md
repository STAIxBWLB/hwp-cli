...
| [08-external-research.ko.md](08-external-research.ko.md) | 외부 근거 — 표준·오픈소스·**생태계 기능 대조**(deep-research) | §10 GJ·§15 로드맵의 수요·구현 선례 근거 |
...
어떤 것에도 의존하지 않는다(→ §15 의존 그래프에서 "즉시 착수 가능" 노드). **GE-β는 "충실도 보존
...
틀. 현재 hwp-cli가 가진 것은 `OfficialPreset::{Gian, Report}`뿐으로 글꼴·여백·4단계 번호·쪽번호만
설정한다(`hwp-convert/src/from_markdown.rs:348-386`). 규정 계층은 이 바이너리를 외부에서 구동하던
워크스페이스 로컬 Python 스킬(`hwpx`)에 있었다. 2026-08-20 결정은 그 계층을 **네이티브로 흡수**하는
것이다 — 스킬의 산문·규정 참고문서·템플릿을 바이너리에 내장하고, 그 기능은 hwp 명령·플래그가 된다.
이 계열을 낳은 대조는 kordoc v4.9.0(MIT)이며([08](08-external-research.ko.md)), 스펙과 단계 분할은
[이슈 #121](https://github.com/STAIxBWLB/hwp-cli/issues/121)이다.

| ID | 현상 | 비고·선례 | 난이도 |
|---|---|---|---|
| GN-1 | **항목 부호가 4단계에서 멈춤** — 공문서 프리셋이 모든 번호 수준을 `1. / 가. / 1) / 가)` 순환으로 채워, 5~8단계에서 법정 부호 `(1) / (가) / ① / ㉮`가 나오지 않음 | `from_markdown.rs:372-384`(`i % 4`). 렌더러는 이미 7단계 기본값을 알고 있다(`hwp-model/src/list.rs:75-92`). IR에 원문자 한글 `NumFmt`를 추가하고 hwp5·hwpx 매핑을 스펙 번호 표에서 가져와야 한다(정품 바이트 확보 우선, 없으면 정답지 대기 표기). 법정 근거: 부호와 코드포인트(`①` U+2460, `㉮` U+326E, `(1)`은 3글자)와 '하' 이후 단모음 연속 | M |
| GN-2 | **공문서 프리셋이 2종뿐** — 기안문·보고서만 있고 계획서·공고문·회의록·보도자료는 본문 크기·불릿 관행·여백·틀이 다름 | 프리셋 계열 확장 + 한국어 알리아스. kordoc은 실측 기본값을 가진 7종을 제공한다(`src/hwpx/gongmun.ts:172-197`) | S |
...
| GN-9 | **프리셋 위 여백 30mm 근거 없음 — 편람 20/10/20/20과 불일치**: `gian`/`report` 프리셋은 위 30 / 아래 15 / 왼쪽 20 / 오른쪽 15 mm를 고정하지만, kordoc 법규 편집본(`gongmunseo-reference.md` §3.2)은 "위 30mm"가 어느 권위 출처에도 없다고 반증한다. 2020 행정업무운영 편람의 공식 조합은 위 20 / 아래 10 / 좌 20 / 우 20 | [23-hwpx-skill-absorption](23-hwpx-skill-absorption.ko.md) §여백 점검(D-14)에 기록. 프리셋 튜플은 `from_markdown.rs:1534-1540`. 판정 2026-08-20: 근거 없음 → 이 행을 개설. **2.1에서 프리셋 변경 없음** — 교정은 Phase 2.2(writer 변경 ⇒ 한컴 검수, 07 PROC). 상태: open — 1차 원문 체크포인트 승인 2026-08-21(판정 유지, 30mm 인용 미제시), 교정은 Phase 2.2로 이연 | S |

GN-1~GN-8은 삽입 단계 2.1~2.5로 편성돼 있다(스펙 이슈 참조). GN-9는 2026-08-20 D-14 여백
점검에서 추가됐고 phase 2.2 대기다. ✅ GN-7은 해소됐다(2026-08-20, phase 2.1). 나머지는 open이다.

## 15. 로드맵 — 난이도 × 가치 + 의존 그래프
