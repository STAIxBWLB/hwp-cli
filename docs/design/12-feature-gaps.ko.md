...
| GN-3 | **표기법 검사 없음** — `hwp validate`는 구조 검증(mimetype·엔트리·XML)뿐이고 규정 표기법 룰셋이 없다: `2026. 8. 20.` 대신 `2026.8.20`, `14:00` 대신 `오후 2시`, `345천원`, 쌍점이 붙은 `붙임:`, 누락된 `끝.` | 조언용 `hwp lint`(규칙 표, `--strict`, 마크다운·문서 입력) + MCP 툴. kordoc `src/hwpx/gongmun-lint.ts`가 선례(15개 규칙, 그 자체가 `jkf87/hwpx-skill` 파생) | S |
...
| GN-8 | **퇴역 스킬과의 편집 동등성 미증명** — 워크스페이스 스킬은 run을 넘나드는 슬롯을 채우고 줄배치 캐시를 지우고 라벨/값 양식을 채웠으나, 대응하는 네이티브 경로가 그 템플릿들에서 아직 실증되지 않음 | 구 스킬 퇴역([skills#35](https://github.com/STAIxBWLB/skills/issues/35)) 전에 `fill`/`edit` 동등성을 증명하고 빠진 것(최소한 라벨 지정 셀 채우기)을 채운다 | S |
...
지름길)·GN-6(표 스타일링)은 phase 2.4에서 해소됐다. GN-3(lint)·GN-8(편집 패리티)은
배정된 phase에서 계속 open 상태다.
...
|---|---|---|---|
| **가치 高**(빈출) | **GN-2·GN-3·GN-5·GN-6·GN-8**(공문서 프리셋·표기법 lint·문서 템플릿·표 서식·편집 동등성 — 2026-08-20 공문서 계열). ✅해소 2026-08-20 (phase 2.1): ~~GN-7~~(번들 규정 계층 출하). GC-4·GC-5(탭·구역속성), GC-8·GC-9(내어쓰기·문단배경) — ✅해소(2026-07-15): ~~GE-α1~α5·α7, GH-1·GH-2, GL-1, GA-5, GE-β4~~ / ✅해소(2026-07-18, md): ~~GH-3·GH-4·GH-5·GH-6, GH-8~~ | **GN-1·GN-4**(법정 8단계 부호·문서 틀 — writer 산출물, 한컴 검수), GG-3·GG-4(양쪽정렬·자간), GF-2(찾아보기·겹침), **GA-2★**(배포용 읽기 — 공식 스펙 공개), ~~GJ-1 출력~~(DOCX 내보내기 2026-08-01 해소), **GK-1**(셀 병합), **GK-2**(열 삭제 — 추가는 07-19 해소) — ✅GC-2·GC-3은 07-19 해소(J1 실기 대기) | GG-1·GG-2(글상자 드롭·오버플로) |
