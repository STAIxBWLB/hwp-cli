...
틀. Phase 2.2는 일곱 표준 프로필, 검증된 8단계 번호, 문서화한 여백 계열을 출하했으며 규정 계층은
...
| GN-2 | ✅ **해소 2026-08-22 (phase 2.2)** — 일곱 표준 공문서 프로필: `official`, `report`, `plan`, `notice`, `minutes`, `gaejosik`, `press` | 표준 별칭과 한국어 이름은 라이터 선택 전 정규화한다. 모든 프로필의 20/10/20/20 여백과 본문·머리말/꼬리말·쪽 번호 기본값을 정품 한컴 오피스에서 관찰했다. `gaejosik`의 맑은 고딕 선택은 실무상 가정으로 유지 | S |
