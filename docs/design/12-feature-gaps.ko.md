...
| ID | 현상 | 메모·선례 | 난이도 |
|---|---|---|---|
| GO-1 | **세그먼트 봉투가 문단 단위뿐** — `hwp cat --with-segments`는 `kind: "para"` 세그먼트와 section/para 인덱스만 방출. run/table/cell/image/field 종류, 안정 ID, 스타일 요약, 스키마 없음 | hwp-editor `packages/core/src/segments.ts`가 정확히 이 형태를 파싱. 버전드 v2 봉투(`schemas/segment-envelope-v2.schema.json`, 플래그 게이트)로 v0.8.x 출력을 한 번의 릴리스 동안 기본값으로 유지 | S |
