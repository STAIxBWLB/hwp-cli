...
| GO-2 | **2026-09-20 해소(Phase 5).** 기존: 렌더러는 픽셀/SVG/PDF만 방출하고 기계판독 가능한 바울딩 박스가 없어, 에디터가 클릭을 세그먼트에 매핑(히트 테스트)·선택 오버레이·스크롤 동기화를 할 수 없었다. 현재 `hwp render --layout-json <path>`가 세그먼트 ID 조인 키를 담은 페이지별 지오메트리 행을 기록하며(고유 키가 아님: 스키마는 `id`와 `(page, id)`가 모두 고유하지 않음을 공표한다. 세그먼트 엔벨로프와의 조인은 셀 문단·중첩 표를 포함한 본문과 그 표 트리에서 양방향으로 완전하며, 아무것도 그리지 않은 문단(모든 줄이 표 셀 경계에서 잘렸거나 개체가 그려지지 않음)도 상자가 null인 행 하나를 유지한다 - #283, #285), `schemas/render-layout-v1.schema.json`으로 공표됐고, 행은 기존 display-list/레이아웃 패스에서 나온다 | `hwp render --layout-json <path>`가 세그먼트 ID 조인 키의 페이지별 지오메트리 행을 기록(`schemas/render-layout-v1.schema.json`). 행은 기존 display-list/레이아웃 패스에서 나온다 | M(완료) |
