...
| [08-external-research.ko.md](08-external-research.ko.md) | 외부 근거 — 표준·오픈소스·**생태계 기능 대조**(deep-research) | §10 GJ·§16 로드맵의 수요·구현 선례 근거 |
...
- 갭 ID는 `계열-번호` 형식(`GA-1`, `GB-6`). 계열:
  ...
  - **GN** (2026-08-20 kordoc 대조 추가): 공문서 저작 계층
  - **GO** (2026-08-22 hwp-editor 대조 추가): GUI 에디터 엔진 표면
...
> 2026-07-19 커버리지 감사): GB-13~15·GD-4·GE-9~13·GE-α9·GE-β7~β8·GF-4~5·GG-21~24 추가, GE-4/5/6
> write 측 근거 보강, GG-15/GG-19 범위 확장, GB-8/GF-2 문구 정밀화 → **2026-08-20 kordoc 대조**:
> GN 계열(공문서 저작 계층)과 GM-10(PII 비식별) 추가, GJ-1 들여오기 처분 기록.
...
- **2026-07-18 (4차 — Phase 2 스펙 감사)**: 사용자 재구성 스펙 md 기반 전면 감사(15건 확정,
  ...
  **C9**(표 공통속성 44B→46B), **C10**(쪽나눔 자리 holdAnchor 오기록 제거). 나머지 11건은
  주석·설계문서·스펙 오류로 정정했다(이 가운데 스펙 쪽은
  [19 §1](19-hwp5-spec-supplement.ko.md)에 정오표로 정리되어 있다). 실기 4라운드 끝에 **D1·D2·D3 전부 통과** —
...
해소됐다. 마지막 결함은 암호 알고리즘이 아니라 인코딩에 있었다. 키를 암호의 UTF-8 바이트가 아니라
**CP949** 바이트에서 유도하기 때문에, ASCII 암호는 동작하는데 비ASCII 암호는 전부 거부되고 있었다.
ASCII는 두 인코딩에서 바이트가 같으므로, 코퍼스에 자격 증명이 ASCII가 아닌 fixture가 반드시 필요한
이유가 바로 이것이다([07 H1](07-hangul-compat-rules.ko.md)). HWPX 쪽은 실측으로 확정했고, HWP5 쪽은
같은 가정을 공유한다는 근거로 동일한 후보 목록을 적용했을 뿐이므로, 비ASCII 암호로 저장된 정품 HWP5
문서를 실측하기 전까지는 추론으로 남는다. GA-3·GA-4는 앞서 명시적 거부로 닫았고, 인증서·DRM·서명 비트와 실제 조건의 대응은
...
어떤 것에도 의존하지 않는다(→ §16 의존 그래프에서 "즉시 착수 가능" 노드). **GE-β는 "충실도 보존
...
| GF-4 | **필드 1/34종 미인식 → 그 한 종만 hwp5→hwpx에서 통째 DROP** — `is_field_ctrl_id`가 스펙 표 128의 34종 중 33종을 인식. 메모(`%%me`) 한 종만 보류이며 의도적이다: 본문이 메모 리스트(GB-7)와 짝을 이뤄야 하는데 아직 미방출이라, dangling 메모 필드를 내보내면 한글에서 문서가 깨질 수 있다. 그 한 종은 어떤 write arm에도 안 걸려 catch-all DROP → **필드 전체 소실**(GF-1의 "종류 뭉갬"보다 상위 손실 등급). 나머지 33종은 fieldBegin/End와 내용 텍스트로 보존되며, 그중 12종만 named OWPML type을 갖고 21종은 `UNKNOWN`으로 폴백한다(이 행이 아니라 GF-1의 손실 등급). hwp5→hwp5는 무손실(`is_field_ctrl_id`를 hwp5 crate가 안 씀). 2026-07-19 스펙 전수 감사 발견, 인식 목록 확장 이후 2026-09-20 카운트 정정 | `hwp-convert/src/field.rs:37-53` ↔ 게이트 사용처 `hwpx/src/write/section.rs:271`, catch-all `:386-391` | §4.3.10.15 표 128 | 드롭(합성) | 합성(hwp5→hwpx) | S(GB-7이 메모 본문을 방출하면 `%%me` 추가 + `UNKNOWN` 21종의 fieldBegin type 매핑) |
...
| GG-25 | ~~**책갈피·하이퍼링크 컨트롤이 렌더에서 조용히 누락**(문단 단위 책갈피(`bokm`) 또는 하이퍼링크(`%hlk`) 컨트롤이 미지원 렌더 누락으로 집계됐으나, 책갈피는 설계상 비가시 마커이고 하이퍼링크 텍스트는 이미 그려지고 스타일이 적용돼 있었음)~~ | `hwp-render/src/layout.rs:963-980`(렌더 대상 컨트롤 화이트리스트) | ✅ **해소(2026-09-20, PR [#258](https://github.com/STAIxBWLB/hwp-cli/pull/258), #254 해소)**: 기존 `cold`/`head`/`foot`/`fn`/`en` 화이트리스트에 두 `ctrl_id`를 추가. 분류 수정일 뿐 새 그리기 경로는 없음 | S |
| GG-26 | **폰트 무관 페이지 기하 결함**(네이티브 인증의 `outside_page_bounds`/`possible_collision` 검출이 A3(1, 5, 7쪽), A4(20쪽), O_report_hwp/O_plan_hwp 문서(4, 6쪽)에서 테스트한 모든 폰트 후보에 걸쳐 동일하게 유지돼, 폰트 대체 아티팩트가 아니라 문서 기하 원인을 가리킴) | `hwp-render/src/lib.rs:474-497`(`outside_page_bounds`), `:498-540`(`possible_collision`) | ⚠ **부분 해소(2026-09-20, PR [#262](https://github.com/STAIxBWLB/hwp-cli/pull/262), A3 1쪽만)**: 비앵커 도형이 다른 도형의 문단 목록에 중첩된 경우, 저장된 x/y에 부모 도형 자신의 원점을 더하도록 수정해 A3 1쪽의 검출 10건을 모두 제거. A3 5쪽(고정 높이 글상자가 중첩 문단 64개 중 약 44개를 넘어 오버플로, 이미 이연된 §F1/§F2 갭과 연결), A3 7쪽=A4 20쪽(저장된 과대 크기 `Picture` 표시 박스, 렌더러 버그 아님), O_report_hwp/O_plan_hwp 4/6쪽(체계적인 마지막 흐름줄 페이지 나눔 초과)은 미해소로 남아 `CHANGELOG.md` "Known issues"에 대체 폰트 측정 단서와 함께 선언, 정품 한컴 페이지네이션 정답지 확보 후 해소 예정 | M to L |
...
| GH-6 | **리스트 평문화(md)** — 헤딩만 인식, 글머리표/번호 문단을 `- `/`1. ` 구문으로 복원 안 함 | `markdown.rs` + `hwp-model/src/list.rs`(render에서 이동, SSOT) | ✅ **해소(2026-07-18)** — `- `/`N. ` 목록 + 부모 마커 폭 기준 들여쓰기, 정의별 번호 카운터와 구역별 재시작, 번호 형식 합성(숫자 외 엔진 마커는 서수 숫자 마커로 내려 md 왕복 유지, #134) | 내보내기 | S |
...
## 14. GN — 공문서 저작 계층 (2026-08-20 kordoc 대조 신설)

공문서에는 법정 형식이 있다 — 8단계 항목 부호 체계, 날짜·시각·금액·붙임의 표기 규칙, 정해진 문서
틀. Phase 2.2는 여섯 표준 프로필, 검증된 8단계 번호, 문서화한 여백 계열을 출하했으며 규정 계층은
계속 번들 스킬로 바이너리에 담긴다. 남은 갭은 표기법 lint, 문서 프레임, 템플릿 지름길, 표 스타일링,
편집 패리티다. 이 계열을 낳은 대조는 kordoc v4.9.0(MIT)이며([08](08-external-research.ko.md)),
스펙과 단계 분할은 [이슈 #121](https://github.com/STAIxBWLB/hwp-cli/issues/121)이다.

| ID | 현상 | 비고·선례 | 난이도 |
|---|---|---|---|
| GN-1 | ✅ **해소 2026-08-22 (phase 2.2)** — 모든 표준 프로필이 가시 8단계까지 법정 `1. / 가. / 1) / 가) / (1) / (가) / ① / ㉮` 사다리를 렌더링하며, 작성 깊이 9 이상은 실패-폐쇄적으로 거절 | HWPX는 `CircledHangul` 매핑을 직접 쓴다. HWP5는 지원 근사 또는 literal/raw-byte 주장이 아닌, 정품 한컴 오피스에서 관찰된 safe/direct 인코딩을 쓴다. 한컴 관찰은 14/14 HWP/HWPX 프로필 파일에서 2·6·8단계 `하` 이후 이어쓰기, 원본 순서, 가시 기호를 확인 | M |
| GN-2 | ✅ **해소 2026-08-22 (phase 2.2)** — 문서 유형마다 하나씩 두는 표준 공문서 프로필: `official`, `report`, `plan`, `notice`, `minutes`, `press` | 표준 별칭과 한국어 이름은 라이터 선택 전 정규화한다. 모든 프로필의 20/10/20/20 여백과 본문·머리말/꼬리말·쪽 번호 기본값을 정품 한컴 오피스 14개 파일에서 관찰했다. **2026-08-22 개정**: 한국공공언어진흥원 「공문서 작성법 길라잡이」(2025-01-08)가 개조식을 문서 종류가 아니라 내부결재 문서의 본문에서 쓰는 명사형 종결 방식으로 규정하므로 일곱 번째 프로필 `gaejosik`을 폐지했다. 산출 헤더가 `notice`와 동일했고 맑은 고딕 선택에는 근거가 없었다. 개조식은 스킬 규정 참고 문서 §6 어투에 남는다 | S |
| GN-3 | ✅**해소 2026-08-23 (phase 2.3)** — ~~**표기법 검사 없음** — `hwp validate`는 구조 검증뿐이고 규정 표기법 룰셋이 없다~~: `hwp lint`가 `.md`·`.hwp`·`.hwpx`와 표준 입력 markdown에 10개 규칙을 적용한다(날짜·시각·금액·`붙임:`의 쌍점과 번호·마무리 `끝.`·문장부호의 표기법 규칙 7개, 장식 기호를 잡는 규칙 1개, 법정 항목 부호와 로마자 제목의 구조 규칙 2개) | `crates/hwp-convert/src/lint.rs`. 기본은 권고이며 `--strict`는 오류 등급이 있을 때 1로 종료하고 `--json`은 `hwp-lint-report-v1`을 출력한다. 오류 등급은 구조 규칙 2개뿐이다. MCP 도구 `hwp_lint`로도 노출 | S |
| GN-4 | ✅ **해소 2026-08-24 (phase 2.4)** — ~~**문서 틀 없음**~~: 기안문 두문(기관명·수신·경유)과 결문(발신명의·결재/협조·시행/접수·연락처 블록), 공고문 머리·꼬리, 보도자료 머리박스가 `hwp new --doc-head`/`--doc-foot`/`--notice-head`/`--notice-foot`/`--press-head`로 생성되며, CLI와 MCP(`hwp_new`) 양쪽에서 접근 가능 | 모든 틀은 앵커 문단 하나에 감싸인 표(`table_paragraph()`와 같은 모양, D-02/D-10). 이번 phase 산출물의 한컴 검수 절차(07 PROC) 완료 | M |
| GN-5 | ✅ **해소 2026-08-24 (phase 2.4)** — ~~**문서 템플릿 없음**~~: `hwp new --template <슬러그-또는-별칭>`이 8개 문서 스켈레톤을 직접 내장 로드하고(`--list-templates`로 나열), `minutes.md`는 법정 9요소(공공기록물 관리에 관한 법률 시행령 제18조, D-19)를 반영함 | `commands::skill::template_file`로 해석하는 내장 마크다운 스켈레톤 — 기존 `hwp fill` 경로에 연결. `--from`과 모든 프레임 플래그와 상호 배타적(D-05) | S |
| GN-6 | ✅ **해소 2026-08-24 (phase 2.4)** — ~~**생성된 표에 서식이 없음**~~: 모든 GFM 표가 생성 시점에 머리행 음영·굵게·가운데 정렬과 내용 비례 칼럼 폭을 받고(D-07), `hwp edit --style-tables`/MCP `hwp_edit.style_tables`가 기존 파일에도 같은 서식을 적용하며 재적용해도 바이트 안정적(D-08) | 내용 기반 서식(`hwp-convert::style_table`/`style_tables`), 한글 인식 `display_width`. 워크스페이스 스킬의 `style_pass.py`와 그 코퍼스 근거가 출처였음 | S |
| GN-7 | ✅ **해소 2026-08-20 (phase 2.1)** — ~~**규정 계층이 출하되지 않음**~~: 공문서 가이드·규정 참고문서·템플릿이 이제 번들 스킬(`hwp skill export`)에 디렉터리 트리로 담긴다. 영문 정본 + 한국어 미러, 드리프트·EN/KO 패리티 게이트로 고정 | 살아있는 체크리스트와 서브커맨드 단위 패리티 매트릭스는 [23-hwpx-skill-absorption](23-hwpx-skill-absorption.ko.md)에 있다(issue #121). 네이티브 측 능력은 GN-1~GN-6·GN-8로 phase 2.2~2.5에 편성 | S |
| GN-8 | ✅**해소 2026-08-26** — ~~**퇴역 스킬과의 편집 동등성 미증명** — 워크스페이스 스킬은 run을 넘나드는 슬롯을 채웠으나 대응하는 네이티브 경로가 실증되지 않음~~: `hwp fill`이 인라인 서식 때문에 여러 텍스트 run으로 쪼개진 `{{슬롯}}`을 합쳐 채우며, `hwp slots`와 `hwp fill`의 일치는 `crates/hwp-cli/tests/slot_fill_agreement.rs`가 고정한다 | 채운 값은 첫 run의 글자 모양을 상속한다. 슬롯이 줄바꿈이나 문단 경계를 넘지 못하는 것은 의도한 제약이다([skills#35](https://github.com/STAIxBWLB/skills/issues/35)) | S |
| GN-9 | ✅ **해소 2026-08-22 (phase 2.2)** — 근거 없는 30/15/20/15 프리셋 조합을 모든 표준 프로필의 위/아래/왼쪽/오른쪽 20/10/20/20 mm로 교체 | [23-hwpx-skill-absorption](23-hwpx-skill-absorption.ko.md) §3에 D-14 근거 경계를 기록한다. 정품 한컴 HWP/HWPX 14/14 관찰에서 프로필 레이아웃을 검증했다. 변별 CLI/MCP 여백 override는 법정 주장이 아니라 호출자 제어로 남는다 | S |

GN-1, GN-2, GN-9는 phase 2.2에서 해소됐고 GN-7은 phase 2.1에서, GN-4(프레임)·GN-5(템플릿
지름길)·GN-6(표 스타일링)은 phase 2.4에서, GN-3(lint)은 phase 2.3에서, GN-8(편집 패리티)은
2026-08-26에 해소됐다. GN 계열은 전부 닫혔다.

## 15. GO — GUI 에디터 엔진 표면 (2026-08-22 hwp-editor 대조 신설)
...
| ID | 현상 | 메모·선례 | 난이도 |
|---|---|---|---|
| GO-1 | **2026-09-20 해소(Phase 5).** 기존: 세그먼트 봉투가 문단 단위뿐으로 `kind: "para"`와 section/para 인덱스만 방출. 현재 `hwp cat --with-segments --segments v2`가 7종 전부를 파생 ID·문자 범위·두 계층 스타일 요약과 함께 방출하며 `schemas/segment-envelope-v2.schema.json`으로 공표. v0.8.x 출력은 한 번의 릴리스 동안 기본값으로 남으며 골든으로 고정 | hwp-editor `packages/core/src/segments.ts`가 파싱하는 v1 형태는 그대로. ID는 **의도적으로 유일하지 않다**: 블록이 끊은 run은 연속 조각마다 한 번씩, 모든 조각이 그 run의 단일 ID를 갖는다 | S(완료) |
| GO-2 | **2026-09-20 해소(Phase 5).** 기존: 렌더러는 픽셀/SVG/PDF만 방출하고 기계판독 가능한 바울딩 박스가 없어, 에디터가 클릭을 세그먼트에 매핑(히트 테스트)·선택 오버레이·스크롤 동기화를 할 수 없었다. 현재 `hwp render --layout-json <path>`가 세그먼트 ID 조인 키를 담은 페이지별 지오메트리 행을 기록하며(고유 키가 아님: 스키마는 `id`와 `(page, id)`가 모두 고유하지 않음을 공표한다. 세그먼트 엔벨로프와의 조인은 본문, 모든 깊이의 표, 그리기 개체의 텍스트와 그 안의 표까지 어디서나 양방향으로 완전하며, 아무것도 그리지 않은 문단(모든 줄이 표 셀 경계에서 잘렸거나 개체가 그려지지 않음)도 상자가 null인 행 하나를 유지한다 - #283, #285, #350), `schemas/render-layout-v1.schema.json`으로 공표됐고, 행은 기존 display-list/레이아웃 패스에서 나온다 | `hwp render --layout-json <path>`가 세그먼트 ID 조인 키의 페이지별 지오메트리 행을 기록(`schemas/render-layout-v1.schema.json`). 행은 기존 display-list/레이아웃 패스에서 나온다 | M(완료) |
| GO-3 | **2026-09-20 해소(Phase 5).** 기존: 래스터 포맷이 PNG/SVG뿐이라 hwp-editor의 `PageImageFormat`은 jpeg/webp를 포함하나 서버가 `cli-engine.ts:366`에서 거부했다. 현재 기존 래스터 경로가 `--format jpeg`·`--format webp`를 받아들이며 PR #273으로 main에 머지됐고, jpeg 품질 90·무손실 webp는 Phase 5 D-10a 결정을 따른다 | 기존 래스터 경로에 `--format jpeg|webp` 추가(PR #273, jpeg 품질 90·무손실 webp) | S(완료) |
| GO-4 | **2026-09-23 해소(Phase 6).** 기존: 편집 op가 구분자 결합 argv라 `=>`·`=`·`:`를 포함하는 값은 모호했고(hwp-editor `ops.ts`가 스스로 경고를 명시), 자유 텍스트 앵커는 중복 텍스트에서 충돌했다. 현재 `hwp edit --ops <file.json>`가 `schemas/edit-ops-v1.schema.json`으로 공표된 타입드 op 배열을 적용하며 PR #297로 main에 머지됐고, op별 결과는 `edit-report-v1`이 담는다 | `hwp edit --ops <file.json>`가 기존 28종 op를 포함하는 타입드 op 배열(`schemas/edit-ops-v1.schema.json`)을 적용. PR #297로 머지 | S(완료) |
| GO-5 | **2026-09-22 해소(Phase 7).** 기존: 표 op는 이미 `table:row:col` 숫자 좌표로 주소를 지정했지만 표 밖의 op는 텍스트 앵커만 겨냥해 run 범위 문자 속성, 문단 이동, 리스트 들여/내어쓰기, 문단 스타일을 표현하지 못했다. 현재 `set_format`·`set_para`·`set_align`·`replace`·`insert_para`·`delete_para`가 기존 anchor/pattern에 더해 세그먼트 ID / `section:para` / run 범위 `address` 선택자를 대안으로 갖췄고, 새 address 전용 `move_para`·`indent_para`·`outdent_para` kind가 추가됐다. 같은 텍스트가 두 번 나오는 문서에서 addressed op가 지정한 항목만 건드림을 실증. 구역 간 문단 이동은 명시적으로 범위 밖(D-17, 후속 과제로 기록)이며, 새 kind의 MCP `tool_edit` 노출은 이 phase가 아니라 Phase 9의 몫(D-16) | GO-4 채널 위의 세그먼트 ID / `section:para` / run 범위 주소 지정. 같은 텍스트가 두 번 나오는 문서에서 실증 | M(완료) |
| GO-6 | **2026-09-22 해소(Phase 7).** 기존: `hwp edit`는 적용 개수와 미적용 요청을 stderr에 보고하고 `--allow-partial` 없이는 실패-폐쇄적으로 동작했지만, 기계가 읽는 리포트도 변경 세그먼트 ID도 없고 쓰기 없이 미리보기를 할 수도 없었다. 현재 `hwp edit --report <path>`가 각 op의 상태·건드린 조각 수·변경된 segment id(전/후 쌍, 어느 한쪽의 `null`이 생성 또는 삭제를 표시)를 담은 `edit-report-v1` JSON 파일(`schemas/edit-report-v1.schema.json`)을 쓰고, `--dry-run`은 배치 전체를 실제 writer·검증 경로로 적용한 뒤 게시 대신 폐기해 리포트가 대상 실패와 결과 ID에 대해 진실하도록 한다. GO-5와 마찬가지로 MCP `tool_edit` 노출은 이 phase 밖(D-16) | `hwp edit --report <path>`(적용/실패 op + 변경 세그먼트 ID)와 `--dry-run` | S(완료) |

GO-2와 GO-5는 레이아웃 좌표와 run 주소 지정을 정품 파일로 확인해야 하므로 M이고, 나머지는
기존 경로 위의 자료구조 작업이다. writer를 건드리는 항목(GO-4..GO-6 산출 경로)은 마일스톤 2
phase에 따라 한컴 검수 절차(07 PROC)로 끝낸다.

## 16. 로드맵 — 난이도 × 가치 + 의존 그래프
...
★는 2026-07-08 재평가: **GA-2 배포용은 공식 복호화 스펙 공개로 L→M**(2026-08-20 해소), **GB-1 차트의 hwpx
경로는 OOXML chartSpace라 L→M**. 2026-08-20 추가분은 **GN 공문서 계열**로, 8개 중 6개가 S다 — 규정 계층은 대부분 프리셋·텍스트 규칙·출하 문서이지 포맷 작업이 아니기 때문이다. 정답지와 한컴 검수가 필요한 것은 GN-1(IR과 두 포맷에 새 번호 형식)과 GN-4(writer가 방출하는 틀)뿐이다. 우하단(L·低)은 우선순위 최하.
...
[독립·즉시 착수] ──▶ GE-α6     (HWPX 왕복 중심·step은 ✅해소(2026-08-19), 렌더 centerX/Y·step은 미해소,
                    ...
                    GA-3/GA-4 ✅ (거부 메시지, 2026-08-20 해소 — GA-5 버전 게이트는 ✅해소)
...
[구체 Web consumer] ──▶ GM-9 Web ──선행──▶ protocol core 분리 + artifact model
   ...
   │
[규정 계층 — 대체로 독립] ──▶ GN-7(규칙·계약·템플릿 출하)
                    ──선행──▶ GN-1/GN-2(부호·프리셋)  ──선행──▶ GN-4/GN-5/GN-6
                                    │                            (틀·템플릿·표 서식)
                                    └──▶ GN-3(표기법 lint — 엔진이 아니라 규칙에 의존)
                    ──마지막──▶ GN-8(편집 동등성 증명 후 구 스킬 퇴역)
```
...
- **GN 계열**이 엔진에 의존하는 지점은 GN-1(IR과 두 포맷이 담아야 하는 번호 형식)과
  GN-4(writer가 방출하는 문단·표) 둘뿐이다. GN-7이 먼저인 이유는 그것이 출하하는 규칙이
  나머지를 검사할 기준이기 때문이고, GN-8이 마지막인 이유는 퇴역한 스킬이 하던 일이 하나도
  사라지지 않았다는 증명이기 때문이다.

### 16.3 정답지 선행 항목 (실기·정품 파일 필요)
...
- **GJ-2, GJ-3**: 공식 스펙으로 착수 가능하되, 스펙-실파일 불일치 사례가 알려져 있어
  ([08](08-external-research.ko.md) — 단 정의 14 vs 16B) 정품 코퍼스 검증을 병행. GA-2가 정확히
  이 경로를 밟아 해소됐다. 공개된 알고리즘을 구현한 뒤 정품 배포용문서 11건 전부로 검증했다
...
이 절 전체의 비용을 낮출 검증 인프라가 두 가지 있다. 둘 다 2026-08-20 대조에서
드러났고([08](08-external-research.ko.md)) 아직 편성되지 않았다 — 실제 한글로 렌더하고 재추출하는
Windows **한컴 COM 자동화 하네스**(kordoc `bench/hangul-com-pdf.ps1`)는 검수 절차를 수작업이 아닌
반복 가능한 것으로 만들고, 실환경 정부 문서를 모으는 **공개 코퍼스 수집기**(kordoc
`bench/collect-korea-kr.mjs`)는 기존 `HWP_CORPUS_DIR` soak 테스트에 입력을 공급한다. 수집기는 데이터
정책과 양립한다 — 코퍼스는 로컬로만 받고 커밋하지 않는다.
