...
| GN-4 | **문서 틀 없음** — 기안문 두문(기관명·수신·경유·제목)과 결문(발신명의·결재/협조·시행/접수·연락처 블록), 공고문 머리·꼬리, 보도자료 머리박스를 마크다운에 손으로 적어야 함 | 필드를 받는 `hwp new` 플래그. kordoc `src/hwpx/gen-docframe.ts`·`gen-gongmun-extra.ts`. writer 산출물이라 한컴 검수 절차(07 PROC) 필요 | M |
| GN-5 | **문서 템플릿 없음** — 기안문·회의록을 만들려면 뼈대를 처음부터 써야 하고, 회의록 서식은 법정 9요소(공공기록물 관리에 관한 법률 시행령 제18조)를 반영하지 않음 | `{{슬롯}}` 앵커를 가진 내장 마크다운 스켈레톤과 목록 명령 — 기존 `hwp fill` 경로에 연결 | S |
| GN-6 | **생성된 표에 서식이 없음** — 마크다운 들여오기는 균등 칼럼 폭에 머리행 강조가 없어, 공문서 관행(머리행 음영·굵게·가운데, 내용 비례 폭)과 다름 | 프리셋 하 표 서식 + 기존 파일용 플래그. 워크스페이스 스킬의 `style_pass.py`와 그 코퍼스 근거가 출처 | S |
...
GN-1, GN-2, GN-9는 phase 2.2에서 해소됐고 GN-7은 phase 2.1에서 해소됐다. GN-3(lint),
GN-4(프레임), GN-5(템플릿 지름길), GN-6(표 스타일링), GN-8(편집 패리티)은 배정된 phase에서
계속 open 상태다.
