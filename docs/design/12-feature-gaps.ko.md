...
- **2026-08-13 (PDF parity PR 2, [issue #79](https://github.com/STAIxBWLB/hwp-cli/issues/79))**:
  렌더러 쪽 표 쪽 분할 구현. `body_bottom`을 넘는 표가 이제 `Table.attr` bits 0-1(pageBreak
  ...
  `treat_as_char` 표는 "한 글자"라 나누지 않는다(GE-8). 미디어 박스 무소식 클립은 사라지고
  분할은 보고된다(`TableSplitAcrossPages` info, `TableRowTooTallClipped`,
  `TableCellPageSpanClipped`). 페이지를 건너간 문단의 stale-`para_top` 앵커 버그도 수정.
  신규 모델 접근자 `Table::page_break_policy/repeat_header`, `Cell::is_header/vert_align`.
  셀 내부 분할(pageBreak=CELL)은 행 경계 분할로 근사. 잔존: 한컴 검증 라운드
  (제목 줄 자동 반복 정답지)과 다단 인식 표 분할.
