...
- region을 확정하기 전에 확인할 사항. 의도한 region에서 AgentCore와 Quick MCP integration이 모두
  ...
  대비되는지 확인한다.
- request framing에 대한 go-live gate(#318). v0.20.1부터 `hwp serve`는 request body를
  ...
  보낸다. 도구 22개가 돌아오면 framing이 확인된 것이다. `411`이 오면 chunked 전달이므로, go-live 전에
  chunked request decoding을 먼저 넣어야 한다.
