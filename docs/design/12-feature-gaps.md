...
| GE-α6 | **Gradient center and step** | read `read/section.rs` (`parse_gradation` `:1751`, angle plus centerX/centerY and step) ↔ write `write/section.rs` (`write_shape_element` `:1434`, emitting `gr.center_x`, `gr.center_y` and `gr.step` from the IR) | ~~approximated (constant center and steps)~~ ✅ **resolved 2026-08-19** for center and step: Phase 1 FIDL-01, PR #115 made both sides IR-driven; checked in Hancom in the Phase 1 round (2026-08-19, linear fade in both the reference document and our conversion). **Still approximated, outside this row's title**: the gradation `type`. `parse_gradation` `:1757` keeps a single `radial` bool (`!type.eq_ignore_ascii_case("LINEAR")`) and `write_shape_element` `:1435` emits only `LINEAR` or `RADIAL`, so CIRCLE, CONICAL and SQUARE change on round-trip; the Phase 1 record also states that type 2 as radial is unverified against a genuine file. Status owned by [11](11-hwpx-structure-map.md) §(b), corrected first | round-trip (hwpx → hwpx), render | M |
...
[official specification exists, no reverse engineering needed] ──▶ GA-2 ✅ (distribution decryption, 배포용문서 rev1.2)
   ...
   │
[independent, immediately actionable] ──▶ GE-α6    (gradient center and step ✅ resolved 2026-08-19; the gradation `type` stays
                               approximated; α1 to α5, α7 and α8 ✅ resolved 2026-07-15)
