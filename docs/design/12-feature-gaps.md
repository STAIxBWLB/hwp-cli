...
| GC-4 | **Tab definitions lost** (user tab positions and leader characters) | New IR `TabDef`/`TabItem`, hwp5 `parse_tab_def` (§4.2.7, raw kept alongside so identity is unchanged), hwpx `tabPr`/`tabItem` round-trip. ★ The first round of Hancom testing showed a naked tabItem **hangs Hancom**, corrected to the genuine `hp:switch` structure ([07](07-hangul-compat-rules.md) **A11**) | §4.2.7 `TAB_DEF` / `hh:tabPr` | ✅ **resolved and confirmed in Hancom (2026-07-18, fourth round)**: two defects found in Hancom were resolved by bisection and ground-truth comparison, the raw 0x09 hang ([07](07-hangul-compat-rules.md) **A11**) and a bare tab being ignored at zero width (**A12**, in-t emission with derived attributes). ~~Rendering remains outstanding~~ ✅ **resolved 2026-08-19 for structural emission**: Phase 1 FIDL-03, PR #115 applies the tab positions and leader characters at render time (`crates/hwp-render/src/tab.rs`: `tab_stops` `:22`, `next_tab_at` `:93` and `tab_segment_offset` `:119`, with the raw `TAB_DEF` bytes as the fallback), proven by `crates/hwp-render/tests/tab_render.rs` (`leader_dash_reaches_svg_and_pdf_backends`) and `layout.rs` (`leadered_tab_emits_a_dashed_path_between_the_two_x_positions`, `tab_with_no_leader_emits_no_path`). `tab-leader.hwpx` was opened in Hancom in the Phase 1 round with the leader drawn and the number ending at the stop, but **this is not a Hancom acceptance result** and the limits recorded with it stand: "FIDL-03 leader rhythm is measured, not visually signed off" (our pitch equals Hancom's measured 3.12pt, but no side-by-side human comparison of our render against Hancom's was recorded); "Hancom draws leaders as a `TJ` run of repeated glyphs; we stroke a dash pattern" (the rhythm matches, the mechanism does not, `layout.rs` `LEADER_DASH_SCALE` `:365`); and "Kind-specific placement geometry (right/center/decimal) was not compared against Hancom output" (specifically open: where a decimal tab lands with no separator in the run, and how a decimal-aligned number spanning multiple character-shape runs behaves, since only the first run is inspected) | round-trip (hwpx only), render | S |
...
| GE-2 | **A picture is dropped when its binary reference does not resolve**: if `bin_ref` names no stream in the document, the picture is omitted. ~~A reference that *did* name an existing stream could also consume the wrong one, silently falling back to `bin_streams[0]`~~ | `hwp5/src/write.rs` (`bin_stream_index` `:1723`, mirroring `Document::resolve_bin`: `BinRef::Id` through the `header.bin_data` storage id, `BinRef::ItemRef` by trailing-component match against the `BIN_DATA` table) and the drop site `synth_pictures_para` `:1887-1895` (`bin_index` returns `None` -> `PreservationCode::BinaryAssetRemoved`, then `continue`), pinned by `picture_with_unresolvable_reference_reports_binary_asset_removed` `:4664-4699` | §4.3.9.4 pictures | ~~wrong stream substituted (a different logo, seal, signature or photograph)~~ ✅ **resolved 2026-08-19** for the wrong-stream subcase only: Phase 1 FIDL-02, PR #115 resolves the referenced stream through `bin_stream_index` and removed the index-0 fallback, with the reproduction cases pinned as regression tests in `write.rs`; checked in Hancom in the Phase 1 round (the document shows the referenced image, not a fallback). **Residual, accepted rather than closed**: a genuinely unresolvable reference still drops the picture. The writer records the typed `BinaryAssetRemoved` preservation event and skips synthesis, and the regression test above *requires* both that event and `PictureControlRemoved`, so the row stays open on this half. Deliberate: a reported loss is preferable to substituting an unrelated image, and recovery would need a source for bytes the document does not contain | synthesis | S |
...
| GE-α6 | **Gradient center and step** | read `read/section.rs` (`parse_gradation` `:1751`, angle plus centerX/centerY and step) ↔ write `write/section.rs` (`write_shape_element` `:1434`, emitting `gr.center_x`, `gr.center_y` and `gr.step` from the IR) | ~~approximated (constant center and steps)~~ ✅ **resolved 2026-08-19 for HWPX round-trip only**: Phase 1 FIDL-01 made both sides IR-driven and Hancom-checked the linear fade. **Render remains open**: `hwp-render/src/shape_draw.rs:62-66` passes only radial, angle and stops, so centerX/centerY/step have no rendering effect. **Gradation `type` remains partial**: `parse_gradation` `:1757` reduces every non-LINEAR type to one radial bool and the writer emits only LINEAR/RADIAL; CIRCLE, CONICAL and SQUARE change on round-trip. Type 2 as radial remains unverified against a genuine file. Status owned by [11](11-hwpx-structure-map.md) §(b), corrected first | round-trip (hwpx → hwpx), render | M |
...
| GF-4 | **1 of 34 field kinds unrecognized, so that one field is dropped entirely in hwp5 → hwpx**: `is_field_ctrl_id` recognizes 33 of the 34 kinds in specification table 128. Only memo (`%%me`) is withheld, and deliberately: its body pairs with the memo list (GB-7), which is not emitted yet, so writing a dangling memo field could break the document in Hangul. That one kind matches no write arm and hits the catch-all DROP, **losing the whole field** (a higher loss grade than GF-1's "kind flattening"). The other 33 survive as fieldBegin/End plus content text; 12 carry a named OWPML type and the remaining 21 fall back to `UNKNOWN`, which is GF-1's loss grade rather than this row's. hwp5 → hwp5 is lossless because the hwp5 crate does not use `is_field_ctrl_id`. Found in the 2026-07-19 exhaustive audit; the original count was corrected on 2026-09-20 after the recognition list had been extended | `hwp-convert/src/field.rs:37-53` ↔ the gate's use at `hwpx/src/write/section.rs:271`, catch-all `:386-391` | §4.3.10.15 table 128 | dropped (synthesis) | synthesis (hwp5 → hwpx) | S (add `%%me` once GB-7 emits memo bodies, plus the fieldBegin type mapping for the 21 `UNKNOWN` kinds) |
...
| GG-25 | ~~**Bookmark and hyperlink controls silently omitted from render**: a paragraph-level bookmark (`bokm`) or hyperlink (`%hlk`) control was counted as an unsupported render omission, even though a bookmark is an invisible marker by design and a hyperlink's text is already drawn and styled~~ | `hwp-render/src/layout.rs:963-980` (the rendered-control whitelist) | ✅ **resolved (2026-09-20, PR [#258](https://github.com/STAIxBWLB/hwp-cli/pull/258), closes #254)**: both `ctrl_id`s added to the existing whitelist alongside `cold`/`head`/`foot`/`fn`/`en`; a classification fix only, no new drawing path | S |
| GG-26 | **Font-invariant page-geometry defects**: native certification's `outside_page_bounds`/`possible_collision` detections on A3 (pages 1, 5, 7), A4 (page 20) and the O_report_hwp/O_plan_hwp document (pages 4, 6) survive unchanged across every tested font candidate, indicating a document-geometry cause rather than a font-substitution artifact | `hwp-render/src/lib.rs:474-497` (`outside_page_bounds`), `:498-540` (`possible_collision`) | ⚠ **partially resolved (2026-09-20, PR [#262](https://github.com/STAIxBWLB/hwp-cli/pull/262), A3 page 1 only)**: a nested, non-anchored shape's stored x/y now adds the parent shape's own origin, removing all 10 detections on A3 page 1. A3 page 5 (fixed-height text box overflow past ~44 of its 64 nested paragraphs, ties into the already-deferred §F1/§F2 gap), A3 page 7 = A4 page 20 (a stored, oversized `Picture` display box - not a renderer bug) and O_report_hwp/O_plan_hwp pages 4/6 (a systemic last-flowed-line page-break overrun) remain open, each declared in `CHANGELOG.md`'s "Known issues" under the substituted-font measurement caveat pending a genuine Hancom pagination ground truth | M to L |
...
| ID | Symptom | Notes and precedent | Difficulty |
|---|---|---|---|
| GO-1 | **Closed 2026-09-20 (Phase 5).** Was: the segment envelope is paragraph-only, emitting `kind: "para"` with section/para indices and nothing else. `hwp cat --with-segments --segments v2` now emits all seven kinds with derived ids, character ranges and a two-level style summary, published as `schemas/segment-envelope-v2.schema.json`. The v0.8.x output remains the default for one release, pinned by a golden | hwp-editor `packages/core/src/segments.ts` parses the v1 shape, which is unchanged. Note the ids are deliberately **not** unique: a run interrupted by a block is reported once per contiguous piece, every piece carrying the run's single id | S (done) |
| GO-2 | **Closed 2026-09-20 (Phase 5).** Was: the renderer emitted pixels/SVG/PDF but no machine-readable bounding boxes, so an editor could not map a click to a segment (hit-testing), draw a selection overlay or scroll-sync. `hwp render --layout-json <path>` now writes per-page geometry rows carrying a segment-id join key (not a unique key: the schema discloses that neither `id` nor `(page, id)` is unique; the join with the segment envelope is total in both directions everywhere - the body, its tables at any depth, and the text of drawing objects with the tables inside it - and a paragraph that drew nothing - every line clipped at a table cell's edge, or its object drawn as nothing - keeps one row with a null box - #283, #285, #350), published as `schemas/render-layout-v1.schema.json`, with the rows derived from the existing display-list/layout pass | `hwp render --layout-json <path>` writing per-page geometry rows with a segment-id join key (`schemas/render-layout-v1.schema.json`); rows come from the existing display-list/layout pass | M (done) |
| GO-3 | **Closed 2026-09-20 (Phase 5).** Was: raster formats were PNG/SVG only, while hwp-editor's `PageImageFormat` already listed jpeg/webp and its server rejected them at `cli-engine.ts:366`. `--format jpeg` and `--format webp` are now accepted on the existing raster path, landed via PR #273, with jpeg quality 90 and lossless webp per Phase 5 D-10a | `--format jpeg|webp` on the existing raster path (PR #273; jpeg quality 90, lossless webp) | S (done) |
| GO-4 | **Closed 2026-09-23 (Phase 6).** Was: edit ops are separator-delimited argv, so values containing `=>`, `=` or `:` are ambiguous (hwp-editor `ops.ts` documents the caveat), and free-text anchors collide on duplicated text. `hwp edit --ops <file.json>` now applies a typed ops array published as `schemas/edit-ops-v1.schema.json`, merged to main via PR #297, with per-op outcomes carried by `edit-report-v1` | `hwp edit --ops <file.json>` applying a typed ops array (`schemas/edit-ops-v1.schema.json`) covering the 28 existing op kinds; merged to main via PR #297 | S (done) |
| GO-5 | **Closed 2026-09-22 (Phase 7).** Was: table ops already address by numeric `table:row:col` coordinates, but everything outside a table targeted text anchors only, so run-range character formatting, paragraph move, list indent/outdent and paragraph style could not be expressed. `set_format`, `set_para`, `set_align`, `replace`, `insert_para`, `delete_para` gained a segment-id / `section:para` / run-range `address` selector alternative to their existing anchor/pattern, plus new address-only `move_para`, `indent_para` and `outdent_para` kinds, proven on documents where the same text appears twice (an addressed op hits only the named occurrence). Cross-section paragraph move stayed explicitly out of scope (D-17, recorded as a follow-up); MCP `tool_edit` exposure of the new kinds is Phase 9's, not this phase's (D-16) | Segment-id / `section:para` / run-range addressing on the GO-4 channel; proven on documents where the same text appears twice | M (done) |
| GO-6 | **Closed 2026-09-22 (Phase 7).** Was: `hwp edit` reported applied counts and named unapplied requests on stderr and failed closed without `--allow-partial`, but there was no machine-readable report, no changed-segment ids and no way to preview an edit without writing. `hwp edit --report <path>` now writes an `edit-report-v1` JSON file (`schemas/edit-report-v1.schema.json`) naming every op's status, how many pieces it touched and the segment ids it changed as before/after pairs (`null` on either side marking a creation or removal); `--dry-run` applies the whole batch through the real writer and verifier and discards instead of publishing, so the report is truthful about target misses and resulting ids. As with GO-5, MCP `tool_edit` exposure stays out of this phase (D-16) | `hwp edit --report <path>` (applied/failed ops + changed segment ids) and `--dry-run` | S (done) |
...
|---|---|---|---|
| **High value** (frequent) | ✅ the whole official-document GN series is resolved: ~~GN-2, GN-3, GN-5, GN-6, GN-7, GN-8~~ (official presets, notation lint, document templates, table styling, bundled regulation layer, editing parity — 2026-08-20 to 2026-08-26). GC-4 (tabs: round-trip resolved 2026-07-18, render resolved 2026-08-19 for structural emission, kind-specific placement geometry still unverified against Hancom) and GC-5 (section properties), GC-8 and GC-9 (hanging indent, paragraph background). ✅ resolved 2026-07-15: ~~GE-α1 to α5 and α7, GH-1 and GH-2, GL-1, GA-5, GE-β4~~ / ✅ resolved 2026-07-18 (markdown): ~~GH-3, GH-4, GH-5, GH-6, GH-8~~ | **GN-1, GN-4** (statutory eight-level marks, document frames — writer-visible, Hancom acceptance), GG-3 and GG-4 (justify, letter spacing), GF-2 (index marks, overlap), ~~GA-2 ★~~ (reading distribution documents, resolved 2026-08-20), ~~GJ-1 output~~ (DOCX export resolved 2026-08-01), **GK-1** (cell merge), **GK-2** (column deletion; addition resolved on 07-19). ✅ GC-2 and GC-3 resolved on 07-19 (J1 awaiting Hancom) | GG-1 and GG-2 (text box drop, overflow) |
| **Medium value** | GC-6 (multi-column text boxes), ~~GE-2 wrong-stream selection~~ (resolved 2026-08-19; the unresolvable-reference picture drop remains as an accepted typed loss) and GE-3 to GE-6 (columns, number synthesis), GF-1 (%unk), **GB-12** (bibliography), **GE-β1, β2, β5** (preview, scripts, settings), ~~GG-16~~ (local rendering, resolved in PR 9 alongside GG-5, GG-6, GG-8 to GG-11, GG-17, GG-20, GG-21, GG-22 from 2026-08-13/14), ~~GH-3, GH-4, GH-5~~ (footnote markers, merged cells, in-cell blocks; markdown resolved 2026-07-18, html and odt 2026-08-01), ~~GJ-5 and GJ-6~~ (csv, txt, resolved 08-01). ✅ the whole GI series and GE-7 resolved on 07-19. ~~GK-3, GK-4, GK-6, GK-8~~, ~~GM-1, GM-2, GM-5~~, ~~GM-3, GM-4, GM-8~~ (merge, split, compare — shipped in v0.13.0 and spot-checked in Hancom on 2026-08-29; see the rows above), **GM-6** (corrected to already-covered), GM-7 (sealing, resolved 07-16) — ✅ resolved in the 2026-08-01 batch | GB-4 to GB-7 and GB-10 (word art, forms, grouping, memos, master pages), GC-1 (vertical writing), GD-1 to GD-3 (equations; rhwp precedent), **GE-α6** (HWPX round-trip center and step resolved 2026-08-19; center/step rendering remains open; gradation `type` remains approximated, non-LINEAR collapses to RADIAL), GF-3 (field creation), **GB-1 hwpx chart generation ★** (chartSpace; kordoc precedent), **GJ-2 and GJ-3** (hml, HWP 3.x; specifications public), **GG-7, GG-12 to GG-15, GG-18, GG-19** (render pixel comparison), **GE-β3 and β6** (DocOptions, embedded fonts), **GH-7** (ODT layout), **GK-5 and GK-7** (header editing, styles), **GM-10** (PII redaction; kordoc precedent) | GB-2 and GB-3 (OLE, video), **GJ-1 full round-trip** (DOCX import; 2026-08-20 disposition: stays out of scope and is revisited only after every other roadmap item is complete — the same call covers PDF, XLS and XLSX import, with kordoc as the covering tool meanwhile), **GM-9 Web** (authenticated hosted MCP, tenant isolation and operations; Desktop is resolved) |
...
[official specification exists, no reverse engineering needed] ──▶ GA-2 ✅ (distribution decryption, 배포용문서 rev1.2)
   ...
   │
[independent, immediately actionable] ──▶ GE-α6    (HWPX round-trip center and step ✅ resolved 2026-08-19;
                               render center/step remains open; gradation `type` stays approximated;
                               α1 to α5, α7 and α8 ✅ resolved 2026-07-15)
                    GE-2       (local to write.rs; wrong-stream selection became BinRef resolution ✅ 2026-08-19,
                               an unresolvable reference still drops the picture as a reported loss)
                    GA-3/GA-4 ✅ (refusal messages, resolved 2026-08-20; the GA-5 version gate is ✅ resolved)
                    GC-4 render (tab positions and leaders applied in hwp-render/tab.rs, structural emission ✅ 2026-08-19,
                               kind-specific placement geometry not compared against Hancom; the round-trip is ✅ resolved)
                    (✅ resolved: GH-1/GH-2, GL-1, GC-4 round-trip, GC-5/8/9, GE-β5; GM-7 implemented and awaiting Hancom)
                    (a node without a leading ✅ is not fully closed: read its row for the residual)
   │
...
(export only; input remains L-tier) — the next large item was the GM-3/4/8 family, spot-checked in Hancom on 2026-08-29; the milestone now moves to the release gate. Milestone 1 then closed the Phase 1 fidelity trio (GE-α6, GE-2 and GC-4 render, PR #115 on 2026-08-19) to the extent the code proves: HWPX round-trip gradient center and step, picture stream selection, and structural tab and leader emission. Four residuals stay open in their rows rather than being folded into the closure: gradient center/step rendering, the gradation `type`, the unresolvable picture reference, and kind-specific tab placement geometry. That leaves the release gate itself as the remaining work before v1.0: readiness records, claim discipline, and the catalog sweep this section reflects.
