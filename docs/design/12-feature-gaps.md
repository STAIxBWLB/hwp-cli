...
| GG-18 | **Line spacing model approximated (synthesis only)**: decided by attr1 & 0x3, treating fixed (1) and minimum (3) identically and misreading margin-only (2) as a ratio. Real files use the cached lineseg and are unaffected | `hwp-render/src/lineseg.rs:261-287`, `hwp-model/src/header.rs` (`line_spacing_type`) | ✅ **resolved (2026-08-14, PR 7)**: version-aware `line_spacing`/`line_spacing_type`; ratio `base*v/100`, fixed exact `v/2` (clamp removed), margin-only `base + v/2`, minimum `max(base, v/2)` (per-line natural height approximated by `base`, flagged for the Hancom round) | M |
...
| GG-20 | **Inline control character widths ignored**: fixed-width spaces, hyphens, grouped spaces and the like were not reflected in width computation | `hwp-render/src/shape.rs` (piece loop, `fw_space_run`) | ✅ **resolved (2026-08-14, PR 7)**: HYPHEN shapes a real `-`, NB_SPACE takes the space advance (no wrap opportunity — the greedy glyph breaker never treated spaces as break points anyway), FW_SPACE gets a fixed 1em advance | S |
...
- **GG-7, GG-13 to GG-15, GG-19**: settled by pixel comparison with genuine renders (GG-12 resolved in PR 3, GG-18 in PR 7)
