...
| GO-2 | **Closed 2026-09-20 (Phase 5).** Was: the renderer emitted pixels/SVG/PDF but no machine-readable bounding boxes, so an editor could not map a click to a segment (hit-testing), draw a selection overlay or scroll-sync. `hwp render --layout-json <path>` now writes per-page geometry rows carrying a segment-id join key (not a unique key: the schema discloses that neither `id` nor `(page, id)` is unique, and that an unresolved row id is always a para row with no character range), published as `schemas/render-layout-v1.schema.json`, with the rows derived from the existing display-list/layout pass | `hwp render --layout-json <path>` writing per-page geometry rows with a segment-id join key (`schemas/render-layout-v1.schema.json`); rows come from the existing display-list/layout pass | M (done) |
