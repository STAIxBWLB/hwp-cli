...
| GG-12 | ~~**Outline numbering not rendered**: markers are generated only for head_type 2 (number) and 3 (bullet), excluding 1 (outline)~~ | `hwp-model/src/list.rs` (`ListState::marker_for_render`) | ✅ **resolved (2026-08-13)**: outline paragraphs (head_type 1) render the fixed per-level markers (`1.` / `가.` / `1)` / `가)` / `(1)` / `(가)` / `①`) from a dedicated counter family — the outline `numbering_id` is a raw, unnormalized reference. Text converters keep the heading-only mapping via `marker()`, so no double numbering | M |
...
[obtain ground truth]  ──precedes──▶  GB-1 to 7 (object rendering)  ──needs──▶  10/11 record structure interpretation
   ...
   │                       GG-7/GG-14·15/GG-18/19 ── pixel comparison with genuine renders
   │
...
- **GG-7, GG-14 to GG-15, GG-18, GG-19**: settled by pixel comparison with genuine renders
