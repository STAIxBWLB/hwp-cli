...
| B9 | Text is **the right width per word but drifts along the line**, overflowing table cells and shifting every wrapped line | Three shaping rules were wrong at once, all invisible on full-width Hangul: (a) 자간 was added as a fixed fraction of the font size instead of scaling each glyph's own advance, (b) `글꼴에 어울리는 빈칸` (CHAR_SHAPE bit25) was ignored, so a space took the font's space glyph instead of the fixed half em Hancom uses when the bit is off, and (c) `useKerning` (bit30) was ignored while rustybuzz enables `kern` by default | `advance × (100 + pct) / 100` per glyph; a space is `0.5 em × 장평` when bit25 is clear; pass `kern=0` to rustybuzz when bit30 is clear | Per-word `pdftotext -bbox` against the Hancom oracle. 20pt 자간 -10%: 4-glyph Hangul word 74.00 vs 74.04pt (both readings agree), the space gap 2.66 → 6.96pt, the whole title line 430.62 → 461.04pt. 11pt Latin `Career` decides between the readings: multiplicative 31.99, additive 29.52, oracle 32.07. An 11pt 자간-0 line fixes the space at 5.49pt = 0.499 em. The public one-page oracle gate improves from bad_pixel 0.0179/MAE 2.87 to **0.0057/0.75** | `hwp-render/shape.rs:letter_spacing_ratio, fixed_space_pt, shape_with_font` |
...
| D11 | A floating seal (`edit --seal`, D1) centred horizontally on "(인)" but **not vertically**: the disc's top sits at the text line's top and it hangs below (owner, Hancom 12.30.0 build 6446, macOS) | Our hwpx writer emitted a negative `<hp:pos>` offset as signed decimal (`vertOffset="-2051"`). Hancom writes a negative offset as its **unsigned 32-bit two's complement**. Working hypothesis: Hancom does not read the signed form, falls back to 0 and places the seal at the paragraph top. **Open, not established**: nothing yet shows that Hancom rejects the signed form | Every `<hp:pos>` vertOffset/horzOffset goes through `pos_offset` (`v as u32`): -2051 is written as `4294965245`; non-negative values are byte-identical to before. The reader already maps both forms back to i32 (`attr_offset_i32`, i64 then cast) | Owner-authored local files, Hancom 12.30.0 (6446), macOS: a seal set to a PARA-relative -2.81 mm stored `vertOffset="4294966499"` (= -797); the dragged PAPER-relative seal stored `9123`. Local hwpx corpus scan (930 unique files, 688 written by Hancom): Hancom `<hp:pos>` offsets with a `-` sign 0, values at or above 2^31 in 127 files (vertOffset) and 73 files (horzOffset); `<hp:offset x/y>` follows the same convention. Final verdict: the owner's re-observation of the regenerated D1 | `hwpx/write/section.rs:pos_offset` |
