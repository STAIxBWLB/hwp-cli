...
> **색인 유지보수**: 여기의 규칙 ID(A1…E6)는 명세 보완 색인
> [19-hwp5-spec-supplement](19-hwp5-spec-supplement.ko.md)가 인용한다. 규칙을 추가하거나 번호를
> 바꾸면 그쪽 해당 행도 갱신한다.
...
| B9 | 단어 폭은 맞는데 **줄을 따라 어긋남** — 표 셀을 넘치고 줄바꿈 위치가 밀림 | 셰이핑 규칙 세 개가 동시에 틀렸고 셋 다 전각 한글에서는 드러나지 않음: (a) 자간을 글자 자신의 advance에 곱하지 않고 글꼴 크기의 고정 비율로 더함, (b) `글꼴에 어울리는 빈칸`(CHAR_SHAPE bit25)을 무시해, 비트가 꺼졌을 때 한글이 쓰는 고정 1/2 em 대신 글꼴의 space 글리프를 씀, (c) `useKerning`(bit30)을 무시한 채 rustybuzz 기본값으로 `kern`이 켜져 있었음 | 글자마다 `advance × (100 + pct) / 100`; bit25가 꺼져 있으면 빈칸은 `0.5 em × 장평`; bit30이 꺼져 있으면 rustybuzz에 `kern=0` 전달 | 한컴 오라클과 `pdftotext -bbox` 단어 단위 대조. 20pt·자간 -10%: 한글 4글자 단어 74.00 vs 74.04pt(두 해석이 같음), 빈칸 간격 2.66 → 6.96pt, 제목 줄 전체 430.62 → 461.04pt. 해석을 가르는 건 11pt 라틴 `Career` — 곱셈 31.99, 덧셈 29.52, 실측 32.07. 11pt·자간0 줄이 빈칸을 5.49pt(0.499 em)로 고정. 공개 1쪽 오라클 게이트가 bad_pixel 0.0179/MAE 2.87에서 **0.0057/0.75**로 개선 | `hwp-render/shape.rs:letter_spacing_ratio, fixed_space_pt, shape_with_font` |
...
| D11 | 부유 도장(`edit --seal`, D1)이 "(인)"에 가로로는 가운데지만 **세로로는 어긋남**: 원의 윗변이 글줄 윗변에 붙고 아래로 늘어짐(소유자, 한글 12.30.0 빌드 6446, macOS) | hwpx writer가 음수 `<hp:pos>` 오프셋을 부호 있는 10진수로 방출(`vertOffset="-2051"`). 한글은 음수 오프셋을 **부호 없는 32비트 2의 보수**로 기록. 작업 가설: 한글이 부호 형식을 읽지 못해 0으로 되돌리고 도장을 문단 위쪽에 배치. **미해결·미확정**: 한글이 부호 형식을 거부한다는 근거는 아직 없음 | 모든 `<hp:pos>` vertOffset/horzOffset을 `pos_offset`(`v as u32`)으로 방출: -2051 → `4294965245`, 음수가 아닌 값은 이전과 바이트 동일. 리더는 이미 두 형식 모두 i32로 복원(`attr_offset_i32`, i64 파싱 후 캐스트) | 소유자 작성 로컬 파일, 한글 12.30.0(6446), macOS: 문단 기준 -2.81mm로 설정한 도장이 `vertOffset="4294966499"`(= -797)로 저장, 드래그한 용지 기준 도장은 `9123`. 로컬 hwpx 코퍼스 스캔(고유 파일 930개, 한글 저장 688개): 한글의 `<hp:pos>` 오프셋 중 `-` 부호 0건, 2^31 이상 값은 127개 파일(vertOffset)·73개 파일(horzOffset). `<hp:offset x/y>`도 같은 관례. 최종 판정: 재생성 D1에 대한 소유자 재관찰 | `hwpx/write/section.rs:pos_offset` |
...
| # | 증상 | 원인 | 해결 | 정답지 |
|---|---|---|---|---|
| H1 | 한글에서는 한국어 암호로 열리는 정품 문서가 여기서는 모두 거부되며, 암호를 주지 않았을 때와 같은 거부 메시지가 나온다 | 키를 암호의 UTF-8 바이트가 아니라 **CP949 바이트**에서 유도한다. ASCII 암호는 두 인코딩의 바이트가 같으므로, 암호에 비ASCII 문자가 들어가기 전까지는 이 차이가 드러나지 않는다 | 닫힌 순서 후보 목록에서 유도한다. UTF-8을 먼저 쓰고, 암호가 ASCII가 아니면서 대체 문자 없이 인코딩될 때에만 CP949를 뒤이어 시도한다 | 한글 12.30.0.6446(macOS)이 암호 `비밀번호1`로 저장한 정품 ODF 암호화 HWPX는 보호 엔트리 4개 전부가 CP949에서 인증되고 UTF-8·UTF-16LE·UTF-16BE·UTF-8+NUL 어디에서도 인증되지 않는다. 그 밖의 암호화 프로파일은 정상 동작하는 ASCII fixture와 완전히 동일하다(AES-256-CBC, PBKDF2/1024, SHA-256 시작 키, sha256-1k 체크섬) |

인코딩 목록을 넓히는 것이 안전한 이유는 **인증되는 대상을 넓히지 않기** 때문이다. 후보가 몇 개든
단일 후보가 통과해야 했던 경계를 똑같이 통과해야 하며(hwpx는 엔트리별 체크섬, hwp5는 엄격 레코드
동일성), 틀린 키는 어떤 상태도 확정되기 전에 그 경계에서 걸린다. 다만 이 목록이 열린 탐색이 되어서는
안 된다. 암호를 표현할 수 없는 인코딩은 대체 문자로 뭉개진 후보를 내놓는 대신 아무 후보도 내놓지
않는다.

확정된 것은 HWPX뿐이다. HWP5 변환도 같은 `as_bytes()` 가정을 쓰고 있어 동일한 후보 목록을 적용했지만,
비ASCII 암호로 저장된 정품 HWP5 문서를 실측한 바가 없으므로 그쪽은 추론이며
[12 §GA](12-feature-gaps.ko.md)에 그렇게 표시했다.

---

## I. 수식 스크립트 문법(mini-TeX): 맨 앰퍼샌드는 한글의 예약 토큰이지 자유 텍스트가 아니다

| # | 증상 | 원인 | 수정 | 정답지 |
|---|---|---|---|---|
| I1 | 한글 수식에 `&` 문자를 그대로 입력하면 화면에 아무것도 안 보인다. 입력 직후·재열기 후 모두 동일 | 한글 자체 수식 문법이 맨 `&`를 행렬 열 정렬 예약 토큰으로 처리해 행렬 문맥 밖에서는 보이는 글리프를 만들지 않는다. 이 프로젝트의 자체 토크나이저도 동일하게 공백으로 소비한다. 한글 자체 행렬 템플릿이 방출한 맨 `&`(행 구분은 `#`)도 사용자가 직접 입력한 것이 아니라 한글 자신이 쓴 것으로 확인됨 | ASCII 큰따옴표로 감싼 `"&"`를 넣으면 한글이 따옴표를 벗겨내고 리터럴 `&` 한 글자를 표시한다. 한글 문법이 큰따옴표로 감싼 구간을 토큰 스트림이 아니라 리터럴 텍스트로 처리하는 것이다. `tools/gen_effects_cases.py`의 `l1_equation_hwpx`가 이제 이 따옴표 표기를 쓰도록 고쳐, 재생성된 L1 산출물이 정품 한글에서 리터럴 앰퍼샌드를 표시한다. writer는 호출자가 준 스크립트를 그대로 직렬화할 뿐 앰퍼샌드를 다시 쓰지 않는다. 이 프로젝트 자체 mini-TeX 토크나이저(`crates/hwp-render/src/equation.rs:59`, `tokenize`의 `'&'` 분기)는 이 따옴표-리터럴 이스케이프를 파싱하지 않으므로, `hwp render`가 자체 렌더한 결과는 여전히 한글과 다르다. 이 프로젝트 렌더러에 남은 알려진 미해결 격차이며, 이 계획의 범위 밖이다 | 오너가 직접 만든 `equation-ampersand-groundtruth.hwpx`/`.hwp`(`fixtures/hwpx`, `fixtures/hwp5`; 로컬, gitignore 대상, 2026-09-19 저장, 한글 12.30.0 빌드 6446, macOS). 이 프로젝트 자체 리더(`.planning/.../checks/equation_scripts.py`)가 저장된 바이트에서 두 스크립트를 새로 재계산: 수식 1은 두 포맷 모두에서 3글자 문자열 `"&"`; 수식 2는 (직접 타이핑이 아니라) 한글 자체 2x2 행렬 템플릿으로 만든 `{pmatrix{1&2#3&4}}`로, 행렬 안 맨 `&`/`#` 구분자가 한글 자신의 출력임을 확인한다 |

**아직 열려 있는 것:** 오너가 시도한 이스케이프 표기(ASCII 큰따옴표) 하나만 실측했다. 한글이 리터럴 앰퍼샌드에 다른 표기를 받아들이는지는 미확인이다. 입력 중 수식 편집기가 곧은따옴표를 굽은따옴표로 자동 변환해, 오너가 저장 전에 ASCII로 되돌렸다. 그래서 실측된 이스케이프는 구체적으로 ASCII 따옴표 형태 `"&"`이며, 굽은따옴표 형태가 아니다.
