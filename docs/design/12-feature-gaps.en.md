...
|---|---|---|
| [07-hangul-compat-rules](07-hangul-compat-rules.en.md) §F | The **investigation narrative of unresolved issues** found in Hancom (F1 text-box drop, F2 page overflow) | 12 **inherits by link**: it summarizes and points rather than restating the narrative (see §7 GG) |
| [00-overview](00-overview.en.md) §5 | A **summary snapshot** of the current state | 12 expands that snapshot item by item |
| [10-hwp5-structure-map](10-hwp5-structure-map.en.md) §8 | The list of hwp5 records that are **unparsed (Opaque) or raw-preserved** | The **underlying data** for 12 §2 and §3 (what lossless preservation actually loses) |
| [11-hwpx-structure-map](11-hwpx-structure-map.en.md) §5 | The hwpx read/write **symmetry matrix** (unimplemented, information loss, round-trip asymmetry) | The **underlying data** for 12 §2, §4 and §5 |
| [08-external-research](08-external-research.en.md) | External evidence: standards, open source and an **ecosystem feature comparison** | The demand and precedent evidence behind §10 GJ and the §14 roadmap |
...
cited as facts; wording is never reproduced (see the [README](../README.en.md)).
...
- An hwp5 `OpaqueRecord` (the whole subtree preserved; see the status table in
  [10](10-hwp5-structure-map.en.md) §0) **loses no bytes** in an `hwp5 → hwp5` round-trip, so it is
...
- **2026-07-19 (the GI batch)**: **GI-1 and GI-2 resolved**: from_markdown reconstructs
  ...
  42 is a typo; compared against five genuine records, see [07](07-hangul-compat-rules.en.md) **B7**),
...
pyhwp has implemented it since 2014 ([08](08-external-research.en.md), the ecosystem comparison).
...
- **hwp5** preserves the whole subtree as an `OpaqueRecord`, so `hwp5 → hwp5` is lossless (see the
  Opaque list in [10](10-hwp5-structure-map.en.md) §8).
- **hwpx read** falls back to `GenericControl`, discarding the object's own properties and keeping
  **only the child subList text** in the IR ([11](11-hwpx-structure-map.en.md) §3.3).
...
|---|---|---|---|---|---|---|
| GB-1 | **Charts** (`CHART_DATA` 0x5F / `hp:chart` ooxmlchart) | hwp5 `body_text.rs:617` (Opaque), hwpx unimplemented `write/section.rs:364` (DROP), [11](11-hwpx-structure-map.en.md) §5(c) | §4.3.9.6 | hwp5 Opaque preserved / hwpx dropped (no text either, that is a total loss) | round-trip (hwpx only), synthesis, render | L; hwpx generation **M** ★ |
| GB-2 | **OLE objects** (`SHAPE_COMPONENT_OLE` 0x54 / `hp:ole`) | hwp5 `body_text.rs:617`, hwpx `write/section.rs:364`, [10](10-hwp5-structure-map.en.md) table B | §4.3.9.5 | hwp5 Opaque preserved / hwpx dropped | round-trip (hwpx only), synthesis, render | L |
...
| GB-6 | **Grouped objects** (`SHAPE_COMPONENT_CONTAINER` 0x56 / `hp:container`): ★ **asymmetric**, because hwp5 raw-preserves it and therefore **even renders it** (recursing into children) while hwpx falls back then drops | hwp5 rendering in `hwp-render/src/shape_draw.rs` ([10](10-hwp5-structure-map.en.md) §8 raw-preserved), hwpx `read/section.rs:191` → `write/section.rs:364` | §4.3.9.7 | hwp5 raw-preserved (renders) / hwpx dropped | round-trip (hwpx only), synthesis | M |
| GB-7 | **Memos** (`MEMO_LIST` 0x5D in the body plus `MEMO_SHAPE` 0x5C in DocInfo / no `hp:` emission in hwpx) | hwp5 `body_text.rs:617`, `doc_info.rs:148` (Opaque); hwpx declares only the namespace ([11](11-hwpx-structure-map.en.md) §2) | §4.3 (memos), §4.2 table 13 | hwp5 Opaque preserved / hwpx unimplemented | round-trip (hwpx only), synthesis, render | M |
| GB-8 | **Change tracking and edit history** (`TRACKCHANGE` 0x20, `TRACK_CHANGE` 0x60, `TRACK_CHANGE_AUTHOR` 0x61, `PARA_RANGE_TAG` 0x46 / hwpx `hhs:` history). PARA_RANGE_TAG's purpose in the specification also covers **range marking such as highlighting and proofreading marks** (§4.3.5), not only change tracking | hwp5 `doc_info.rs:148`, `body_text.rs:73` (Opaque); hwpx unimplemented ([11](11-hwpx-structure-map.en.md) §5(c)) | §4.2 table 13, §4.3.5 | hwp5 Opaque preserved / hwpx unimplemented | round-trip (hwpx only), synthesis | L |
| GB-9 | **Arbitrary document and distribution data** (`DOC_DATA` 0x1B, `DISTRIBUTE_DOC_DATA` 0x1C, `COMPATIBLE_DOCUMENT` 0x1E, `LAYOUT_COMPATIBILITY` 0x1F) | hwp5 `doc_info.rs:57` (Opaque), though the writer **synthesizes** COMPATIBLE and LAYOUT separately ([07](07-hangul-compat-rules.en.md) A4) | §4.2.12 to §4.2.15 | hwp5 Opaque preserved (plus synthesis handling) / hwpx unimplemented | synthesis (partly resolved) | L |
| GB-10 | **Master pages** (hwpx `hm:` master-page; no hwp5 counterpart) | absent in both hwpx read and write ([11](11-hwpx-structure-map.en.md) §2, §5(c)) | OWPML master-page | unimplemented | round-trip, synthesis, render | M |
...
comes first ([00](00-overview.en.md) §4). ★ The exception is **the hwpx path of GB-1**: in HWPX a
...
writing infrastructure (precedent: kordoc v3.16, see [08](08-external-research.en.md)).
...
| GC-4 | **Tab definitions lost** (user tab positions and leader characters) | New IR `TabDef`/`TabItem`, hwp5 `parse_tab_def` (§4.2.7, raw kept alongside so identity is unchanged), hwpx `tabPr`/`tabItem` round-trip. ★ The first round of Hancom testing showed a naked tabItem **hangs Hancom**, corrected to the genuine `hp:switch` structure ([07](07-hangul-compat-rules.en.md) **A11**) | §4.2.7 `TAB_DEF` / `hh:tabPr` | ✅ **resolved and confirmed in Hancom (2026-07-18, fourth round)**: two defects found in Hancom were resolved by bisection and ground-truth comparison, the raw 0x09 hang ([07](07-hangul-compat-rules.en.md) **A11**) and a bare tab being ignored at zero width (**A12**, in-t emission with derived attributes). Rendering remains outstanding | round-trip (hwpx only), render | S |
...
| GC-7 | **Odd/even adjustment unparsed**: passes through as Generic without semantic parsing | hwpx `read/section.rs:597` (unknown ctrl → code 21 Generic), [10](10-hwp5-structure-map.en.md) §6.1 footnote | §4.3.10.8 | Generic preserved (unparsed) | synthesis, render | S |
...
Most equations render through the mini-TeX typesetter ([05](05-rendering.en.md), after commit
...
([10](10-hwp5-structure-map.en.md) table B `EQEDIT`), so the gap is **confined to the render path**.
rhwp, an implementation in the same language (Rust), already typesets `MATRIX`, `PMATRIX`, `BMATRIX`
and `DMATRIX` and can serve as a reference ([08](08-external-research.en.md)).
...
in an `hwpx → hwpx` round-trip** ([11](11-hwpx-structure-map.en.md) §5(b)). The shared cause is local
...
| GE-β3 | **DocOptions ancillary streams**: `_LinkDoc` is a 524B zero constant and the six DRM and signature streams are not emitted | `write.rs:208-210`, [10](10-hwp5-structure-map.en.md) §1 | dropped/constant | rewrite | M |
...
All twelve field kinds are parsed on demand ([10](10-hwp5-structure-map.en.md) §6.2), but there are
...
| GF-2 | **Index marks, ruby text and overlapping characters unparsed**: preserved only as Generic without semantic parsing (marker-like controls with no paragraph list; hidden comments were split out as GF-5 because they lose content, 2026-07-19) | hwpx `read/section.rs:597` (unknown ctrl → code 21 Generic), [10](10-hwp5-structure-map.en.md) §6.1 footnote | §4.3.10.10, §4.3.10.12, §4.3.10.13 | Generic preserved (unparsed) | synthesis, render | M |
| GF-3 | **Constraints on creating new fields**: only existing names can be filled, and no new field can be created. Editing can create only `%clk`, `%hlk` and `%bmk`/`bokm` | `hwp-convert/src/field.rs` (limited creation kinds), [README](../README.en.md) scope and limitations | §4.3.10.15 | unimplemented (creation) | editing | M |
...
|---|---|---|---|---|---|---|
| GG-1 (=07§F1) | **Text box drop**: in a round-tripped hwp the text-box frame itself is lost (the text survives by being hoisted into the body) | `hwp5/src/write.rs:467` (`degrade_hwpx_gso`) | Inherited from [07 §F1](07-hangul-compat-rules.en.md). A root fix requires **property fidelity** on the 239B SHAPE_COMPONENT | dropped (safe degradation) | synthesis (hwpx → hwp5) | L |
| GG-2 (=07§F2) | **Page overflow**: synthesized multi-page documents overflow vertically (markdown is defended by the content_h reset) | `hwp-render/src/lineseg.rs` (`synthesize_linesegs`) | Inherited from [07 §F2](07-hangul-compat-rules.en.md). Line layout property fidelity is the leading cause | approximated | render, synthesis | L |
| GG-3 (=U2) | **Justified alignment approximated**: surplus width is distributed to spaces first, assuming a 1:1 glyph-to-character mapping for CJK | `hwp-render/src/layout.rs:386`, [05](05-rendering.en.md) §1.4 (`justify_line`) | With no spaces, the gap before the last glyph is distributed evenly. Mixed non-CJK text introduces error | approximated | render | M |
| GG-4 (=U4) | **Letter spacing approximated**: `spacing_pt = size_pt × spacings[lang]/100` applied naively | [05](05-rendering.en.md):184 (`// 자간`) | Per-language spacing approximated on a pt scale | approximated | render | M |
...
|---|---|---|---|---|
| GG-5 | **Cell border line type ignored**: `BorderLine.line_type` (dotted, double) is unapplied and `Item::Line` has no dash field at all, so every cell border is a single solid line | `hwp-render/src/layout.rs:1080-1091`, `display.rs:32-39`, `hwp-model/src/header.rs:311` | approximated (solid) | S |
| GG-6 | **Paragraph border line type ignored**: the same root as GG-5 on a different path | `layout.rs:1556-1567` | approximated (solid) | S |
...
| GG-12 | **Outline numbering not rendered**: markers are generated only for head_type 2 (number) and 3 (bullet), excluding 1 (outline) | `hwp-render/src/list.rs:17-21` | dropped (no number) | M |
...
| GG-17 | **Column separators not rendered**: `ColumnDef.divider` is parsed but unused (GE-3 is hwpx read not collecting it; this is the hwp5 render path) | `hwp-model/src/control.rs:238`, unreferenced in `layout.rs` | dropped (no line) | S |
...
| GG-21 | **Shape line type and arrowheads unapplied on the hwp5 direct render path**: the `dash_pattern` and `arrowheads` functions exist but are called only on the hwpx ShapeGeom path. The hwp5 raw_children path (`draw_component`) is fixed to `Stroke::solid` and never calls arrowheads, so dotted shapes and arrow lines in an hwp5 original render as plain solid lines. Even the hwpx path flattens arrowhead **shape and size** (table 87 bits 16 to 29) to a fixed triangle. Diagrams and flowcharts hit this often | `hwp-render/src/shape_draw.rs:418` (fixed solid) ↔ `:64`, `:73` (dash and arrowheads on the IR path only) | dropped (solid and plain) | S (port the existing functions) |
...
| GG-24 | **Diagonal cell and paragraph borders and BORDER_FILL effect bits not rendered**: `BorderFill.diagonal` (kind, thickness, color) is parsed but unreferenced by the renderer, and the 3D and shadow effect bits in attr are only raw-preserved (GG-5 and GG-6 cover the four sides; the diagonal is a separate field) | `hwp-model` BorderFill.diagonal ↔ unreferenced in `hwp-render` | dropped (no line) | S (rare) |
...
| GH-3 | **Foot- and endnotes absorbed inline without markers (markdown/HTML/ODT alike)**: `[^n]` and `<text:note>` unused | `markdown.rs`, `html.rs:204-223`, `odt.rs:181-199` | ✅ **markdown resolved (2026-07-18)**: `[^N]`/`[^eN]` markers in the body plus definitions at the end (GFM footnotes). html and odt keep the previous approximation | export | S |
| GH-4 | **Merged cells flattened**: col_span and row_span are unreflected in any output (no colspan/rowspan or columns-spanned emission) | `markdown.rs`, `html.rs:172-203`, `odt.rs:203-243` | ✅ **markdown resolved (2026-07-18)**: with merged cells present, falls back to an HTML `<table>` (colspan/rowspan); a GFM table is kept only for unmerged tables. html and odt keep the previous approximation | export | S |
| GH-5 | **In-cell blocks (nested tables, images) dropped**: a cell takes only inline text and discards the block buffer | `odt.rs:215` (blk discarded), `markdown.rs`, `html.rs:181-189` | ✅ **markdown resolved (2026-07-18)**: falls back to an HTML table when a nested table or block equation is detected, preserving cell fragments in appearance order and referencing images safely. html and odt unchanged. ⚠ Audit note: the code branch (`Control::Table => true`) was verified, but there is no dedicated unit test for "an actual nested table inside a cell" (only indirect verification through equations and images) | export | M |
...
in [08](08-external-research.en.md).

| ID | Symptom | Evidence | Current behavior | Difficulty |
|---|---|---|---|---|
| GJ-1 | **No DOCX input or output**: the most common interoperability request. Demand is high enough that Microsoft ships an official batch converter (HwpConverter plus BATCHHWPCONV), yet OSS HWP → DOCX is open territory | no trace in the code (grep), absent from ConvertFormat in `hwp-cli/src/main.rs` | unimplemented | M to L |
...
| GJ-5 | **No table-to-CSV extraction**: no path to pull tables out as data (quantitative demand evidence is unverified; see the caveat in [08]) | grep finds nothing | unimplemented | S |
| GJ-6 | **`.txt` extension inference fails**: `convert -o out.txt` errors, and plain text is only `cat` to stdout | `hwp-cli/src/commands/convert.rs:195-213` (no txt arm) | unsupported | S |
| GJ-7 | **No reverse input from HTML/ODT/PDF**: input is hwp5, hwpx, json and markdown only (four output-only formats) | `hwp-cli/src/commands/cat.rs:18-44` | unimplemented (one-way) | L |
| GJ-8 | **HWPX distribution documents**: unsupported by every implementation (H2Orestart #42 open). Whether the official HWP5 distribution specification covers the HWPX variant is unverified | [08](08-external-research.en.md), open question | unimplemented | L |
...
| GK-3 | **No new table insertion**: from_markdown creates tables, but there is no anchor-based insertion primitive | - | S |
| GK-4 | **Paragraph shape editing limited to alignment**: no change to line spacing, indentation, left/right margins or paragraph spacing | `format.rs:211-245` (attr1 alignment bits only) | S |
| GK-5 | **No header or footer editing**: only inclusion or exclusion during extraction | `text.rs:62-66` | M |
| GK-6 | **No page setup change**: margins, paper and orientation (PageDef is injected as constants only on `new`) | `from_markdown.rs:562-573` | S |
...
| GK-8 | **No object deletion**: images, fields, tables and bookmarks cannot be deleted (only insertion, and paragraph and row deletion, which is asymmetric) | `edit.rs`, `field.rs`, `image.rs` | S |
...
comparison in [08](08-external-research.en.md).

| ID | Symptom | Demand and precedent | Difficulty |
|---|---|---|---|
| GM-1 | **No batch, glob or directory processing**: every command takes a single file argument | MS BATCHHWPCONV and headless H2Orestart demonstrate batch demand | S |
| GM-2 | **Weak stdin input and stdout piping**: convert and edit require an output file and do not accept `-` (only cat writes to stdout) | Unix CLI convention | S |
...
| GM-5 | **No text search (grep) command**: only edit `--replace` exists | - | S |
| GM-6 | **No bulk metadata editing or dumping**: `--set-meta` is local to new and edit | - | S |
...
- **GB object rendering** requires the record and element structure interpretation of 10 and 11 first
  ...
  **obtaining ground truth first** ([00](00-overview.en.md) §4).
...
methodology in [00](00-overview.en.md) §4 (no guessed typesetting). The rest (especially GE-α, GH, GL,
...
- **GA-2, GJ-2, GJ-3**: can start from the official specification, but because
  specification-versus-real-file mismatches are known ([08](08-external-research.en.md): column
...
that the specification is public), and the largest demand from a product perspective is **GJ-1**
