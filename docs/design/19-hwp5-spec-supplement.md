...
| E-5 | COLDEF column definition (tables 138-139) | 14B | **16B** in real files (external corroboration; our own exhaustive sweep has not been run — settle against ground truth when implementing). Table 138's attribute note also self-references "table 138"; the correct target is table 139 | hwp.js issue #58 | [08 ecosystem](08-external-research.md) |
...
| E-8 | NOTE_SHAPE separator length (§4.3.10.1.2) | `HWPUNIT16` (2B), 26B total | **4B (`HWPUNIT`)**, 28B total | All 476 genuine NOTE_SHAPE records, cross-checked two ways against the hwpx `noteLine@length` attribute (investigation GC-3) | [12 GC-3](12-feature-gaps.md) |
| E-9 | NWNO record (table 144) | Declares an 8B total | The field list sums to **6B**, so the declared total contradicts the table's own fields. Our code follows the fields | Genuine-file measurement during the 2026-07-18 specification audit | [10 §4.1](10-hwp5-structure-map.md) |
| E-10 | PGHD record (table 145) | Declares a length of 2 | The data type column says **UINT (4B)**, so the declared length contradicts the type. Our code follows the type | Genuine-file measurement during the 2026-07-18 specification audit | [10 §4.1](10-hwp5-structure-map.md) |
