...
| 교차변환 | hwp5→hwpx(또는 역) 변환 후 결과의 구조·렌더·strict 검사(행렬에서 4개 열을 차지한다) | 변환 파이프라인 |

## 현재 상태 (2026-08 기준)

한 번 실행하면 **51행**(생성 케이스 22종 × 2포맷 + fixture 7종)에 대해 **10종의 검사**를 수행하며,
**실패하는 검사는 하나도 없다.** 즉 구조·변환·렌더 수준의 문제나 회귀가 없다. 다만 경고 두 가지는
상시 발생하며 예상된 것이다. hwp5 fixture 전부에서 `strict` 변환이 메타데이터 값을 보고하고,
`link` 케이스는 링크 문법이 문자 그대로가 아니라 하이퍼링크로 바뀌기 때문에 마크다운 텍스트의 33%만
보존한다.

이 하네스로 **못 잡는** 것 = **한글(한컴오피스) 특정 렌더 동작**. 예: annual 6쪽 자리표시자
글상자 드롭 + 빈 페이지(한글 미문서화 heavy-content 동작 — 조사 종결·수용, [07](07-hangul-compat-rules.ko.md)).
이런 이슈는 **정품 한글 실기**로만 판정된다. [한컴 실기 검증 체크리스트](../hancom-verification-checklist.ko.md)에
따라 각 파일을 한글에서 직접 열어 확인한다.
