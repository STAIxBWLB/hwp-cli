...
- **첫 줄 들여쓰기를 텍스트 영역 밖에 적용했다(수정 완료).** 정품 `line_seg`의 `horzpos`는
  ...
  **안쪽**의 마커 자리다: 마커는 줄상자 왼쪽, 첫 줄 글자는 그 다음, 이후 줄은 내어쓰기
  ...
  29px 차이).
- **남은 항목:** 줄 안에서 advance가 누적 이탈하고(문자 모양에 따라 좁게도 넓게도 나와 일부
  표 셀을 넘침), 쪽번호 꼬리말이 아예 렌더되지 않으며, 일부 벡터 이미지 텍스트가 페이지
  원점에 놓이고, 이미지 글머리표(`useImage`)가 선언된 문자로 폴백한다. 다음 대상이며,
  raster 지표는 정렬 없이 계산하므로 각 항목이 쪽 전체의 `bad_pixel_pct`·`mae`를 부풀린다.
