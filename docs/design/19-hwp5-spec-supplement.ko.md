...
| E-5 | COLDEF 단 정의(표 138·139) | 14B | 실파일 **16B**(외부 실증. 자체 전수 실측은 미실시 — 구현 시 정답지로 확정할 것). 또한 표 138의 속성 주석은 "표 138 참조"로 자기 자신을 가리키는데, 올바른 대상은 표 139다 | hwp.js 이슈 #58 | [08 생태계](08-external-research.ko.md) |
...
| E-8 | NOTE_SHAPE 구분선 길이(§4.3.10.1.2) | `HWPUNIT16`(2B), 총 26B | **4B(`HWPUNIT`)**, 총 28B | 정품 NOTE_SHAPE 476개 전수 + hwpx `noteLine@length` 속성과의 양방향 대조(조사 GC-3) | [12 GC-3](12-feature-gaps.ko.md) |
| E-9 | NWNO 레코드(표 144) | 총길이 8B로 선언 | 필드 목록의 합은 **6B**이므로 선언한 총길이가 표 자신의 필드와 모순된다. 우리 코드는 필드 합을 따른다 | 2026-07-18 스펙 감사 중 정품 파일 실측 | [10 §4.1](10-hwp5-structure-map.ko.md) |
| E-10 | PGHD 레코드(표 145) | 길이 2로 선언 | 자료형 칸은 **UINT(4B)**이므로 선언한 길이가 자료형과 모순된다. 우리 코드는 자료형을 따른다 | 2026-07-18 스펙 감사 중 정품 파일 실측 | [10 §4.1](10-hwp5-structure-map.ko.md) |
