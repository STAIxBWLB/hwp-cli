...
| `--with-segments` |  |  | 각 출력 문자 범위의 원본 좌표를 한 줄 JSON 봉투로 함께 출력. 버전은 --segments로 고른다 — v1은 markdown 전용, v2는 --format json도 받는다 |
| `--segments` | `v1` \| `v2` | `v1` | --with-segments 봉투 버전. v1은 바이트가 고정된 v0.8.x 봉투(markdown 전용), v2는 7종 세그먼트·안정 id·세그먼트별 스타일을 담고 markdown과 json에서 쓸 수 있다 |
...
| `--out-dir` | `<OUT_DIR>` |  | 출력 디렉터리 (파일명은 "<입력 stem>-NNN.<소문자 입력 확장자>") |
...
| `--doc-foot` | `<DOC_FOOT>` |  | 공문서 결문 블록 "키=값" (키: 발신명의\|기안자\|검토자\|결재자\|협조자\|시행번호\|시행일자\|접수번호\|접수일자\|주소\|홈페이지\|전화\|팩스\|이메일\|공개구분\|수신자, 반복 가능. 수신자는 두문이 "수신자 참조"인 다수 수신 문서의 수신처 목록으로, 지정할 때만 결문 마지막 줄로 나온다) |
...
| `<FILE>` |  |  | 대상 HWP/HWPX 파일 |
| `--json` |  |  | JSON으로 출력 |
| `--forms` |  |  | 한국 공문서 양식 필드도 나열 — 레이블 셀, 본문의 "라벨: 값", 자리표시자를 키·레이블·출처·신뢰도·등장 횟수·필수 여부와 함께 (JSON "fields"; kordoc 규칙) |
...
| `<INPUT>` |  |  | 입력 HWPX 템플릿 |
| `-o, --output` | `<OUTPUT>` |  | 출력 파일 경로 |
| `--set` | `<SET>` |  | 자리표시자 채우기 "이름=값" (반복 가능; `{{이름}}` 치환, 공백을 넣은 `{{ 이름 }}`도 포함). "이름=@부분.md"이면 `{{이름}}` 앵커 문단을 부분 파일(md+HTML 표 블록, 계약 docs/design/18)로 교체 — 대규모 문서의 부분별 조합. "@@"는 리터럴 '@'. 이름은 첫 '='에서 끝나므로 '='가 든 이름은 --data로 넘김 |
...
| `--forms` |  |  | 같은 값으로 한국 공문서 양식 필드도 채움 (hwpx) — 레이블 셀 옆·아래 칸, 본문의 "라벨: 값", "라벨( )" 빈칸, "□항목" 확인란, "(라벨: )" 빈칸. 비어 있지 않은 값 칸과 "라벨:" 뒤 본문 텍스트는 덮어씀(덮어쓸 때마다 경고), 누름틀·그림·표가 든 칸은 쓰지 않으며 값은 그대로 넣음. 키는 공백·쌍점·괄호를 무시하고 비교하며, 보고서에 "unmatched"가 추가됨. 출력은 .hwpx |
