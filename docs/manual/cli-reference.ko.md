...
| `--forms` |  |  | 같은 값으로 한국 공문서 양식 필드도 채움 (hwpx) — 레이블 셀 옆·아래 칸, 본문의 "라벨: 값", "라벨( )" 빈칸, "□항목" 확인란, "(라벨: )" 빈칸. 비어 있지 않은 값 칸과 "라벨:" 뒤 본문 텍스트는 덮어씀(덮어쓴 칸마다 경고), 누름틀·그림·표가 든 칸은 쓰지 않음. 키는 공백·쌍점·괄호를 무시하고 비교하며, 보고서에 "unmatched"가 추가됨. 출력은 .hwpx |
