...
| `--notice-head` | `<NOTICE_HEAD>` |  | 공고문 머리 블록 "키=값" (반복 가능, 키 구성은 이후 계획에서 연결) |
| `--notice-foot` | `<NOTICE_FOOT>` |  | 공고문 꼬리 블록 "키=값" (반복 가능, 키 구성은 이후 계획에서 연결) |
| `--press-head` | `<PRESS_HEAD>` |  | 보도자료 머리 블록 "키=값" (반복 가능, 키 구성은 이후 계획에서 연결) |
