...
- Korean form fields, ported from [kordoc](https://github.com/chrisryugj/kordoc) (MIT, see
  ...
  - `hwp fill --forms` (and MCP `hwp_fill` with `forms`) fills slots and form fields from one
    `--data`/`--set` set of values in one pass (hwpx in, hwpx out). It fills the cell next to or
  ...
  - Each edit splices only the text it replaces, so the value keeps the char shape of that text,
    ...
    `hwp edit` does, but re-serializes only the sections it changed; every other section and
    package entry is copied byte for byte.
  ...
  - Release archives and the claude.ai skill bundle now include `LICENSE-MIT`, `LICENSE-APACHE`
    and `NOTICE`, since the binary carries kordoc's MIT-licensed rules.
...
- A `hwp edit` text replacement (`--replace`, the `replace` op and MCP `hwp_edit`) stored a tab
  ...
  the tab control, CRLF and LF a line break, and other control characters are dropped. The same
  rule applies to `hwp fill --forms` values ([#364](https://github.com/STAIxBWLB/hwp-cli/issues/364)).
- `hwp fill` now fills every slot `hwp slots` lists. Both commands, the MCP `hwp_slots` and
  `hwp_fill` tools, the `--data` table and part fills, part anchors and `hwp template` read a
  slot with one grammar (`hwp_model::slot_tokens`): `{{`, a name, `}}`. The name is any text
...
- Release-readiness run: https://github.com/STAIxBWLB/hwp-cli/actions/runs/36255397533
...
- Every change to the surfaces the 1.0 SemVer scope covers (CLI, MCP tool arguments, schemas)
  is additive or a loosening. Consumers that pin schema hashes should know three schemas changed:
  `render-layout-v1` and `segment-envelope-v2` changed in descriptions only, and
  `render-report-v1` lost upper bounds and gained three issue codes, so every report that
  validated before still validates. Separately, the v2 envelope's content changed, which that
  scope does not cover: it now carries a point `para` segment (`char_range.start ==
  char_range.end`) for a drawn paragraph with no text, and no longer carries a segment for the
  blank line that closes a list. The markdown output is byte-identical.
...
- Release-readiness run: https://github.com/STAIxBWLB/hwp-cli/actions/runs/36197096329
- The private PDF-parity profile excludes four gates. The public one-page gate declares none
  (docs/design/21-pdf-parity.md section 4.3), so these exclusions describe the private profile
  only.
- `fonts`: the document declares its dominant body face through a `substFont`, and the oracle
...
- Release-readiness run: https://github.com/STAIxBWLB/hwp-cli/actions/runs/36148058939
- The private PDF-parity profile excludes four gates. The public one-page gate declares none
  (docs/design/21-pdf-parity.md section 4.3), so these exclusions describe the private profile
  only.
- `fonts`: the document declares its dominant body face through a `substFont`, and the oracle
...
- Release-readiness run: https://github.com/STAIxBWLB/hwp-cli/actions/runs/36109781722
- The private PDF-parity profile excludes four gates. The public one-page gate declares none
  (docs/design/21-pdf-parity.md section 4.3), so these exclusions describe the private profile
  only.
- `fonts`: the document declares its dominant body face through a `substFont`, and the oracle
...
- Release-readiness run: https://github.com/STAIxBWLB/hwp-cli/actions/runs/36083934688
- The private PDF-parity profile excludes four gates. The public one-page gate declares none
  (docs/design/21-pdf-parity.md section 4.3), so these exclusions describe the private profile
  only.
- `fonts`: the document declares its dominant body face through a `substFont`, and the oracle
...
- Release-readiness run: https://github.com/STAIxBWLB/hwp-cli/actions/runs/35934511452
- The private PDF-parity profile excludes four gates. The public one-page gate declares none
  (docs/design/21-pdf-parity.md section 4.3), so these exclusions describe the private profile
  only.
- `fonts`: the document declares its dominant body face through a `substFont`, and the oracle
...
- Release-readiness run: https://github.com/STAIxBWLB/hwp-cli/actions/runs/35823829350
- The private PDF-parity profile excludes four gates. The public one-page gate declares none
  (docs/design/21-pdf-parity.md section 4.3), so these exclusions describe the private profile
  only.
- `fonts`: the document declares its dominant body face through a `substFont`, and the oracle
...
- Release-readiness run: https://github.com/STAIxBWLB/hwp-cli/actions/runs/35551671296
- The private PDF-parity profile excludes four gates. The public one-page gate declares none
  (docs/design/21-pdf-parity.md section 4.3), so these exclusions describe the private profile
  only.
- `fonts`: the document declares its dominant body face through a `substFont`, and the oracle
...
- The release tag gate could not be satisfied by any release. `.github/workflows/release.yml`
  ...
  `Cargo.toml`, `Cargo.lock` and `CHANGELOG.md`, with only `version = "..."` lines changing inside
  the two manifests, so a dependency, feature, workspace-membership or re-lock change cannot ride
  along; the release workflow checks out full history and passes that mode. `scripts/release.sh` keeps the strict equality it always had, because the commit
...
- Native render no longer counts a bookmark (`bokm`) or hyperlink (`%hlk`) control as an
  ...
  already drawn and styled by the existing hyperlink range pass. This is a classification fix
  only - neither control gains a new visual representation
  ([#254](https://github.com/STAIxBWLB/hwp-cli/issues/254)).
- `hwp edit --seal`'s test fixture now carries an alpha channel, fully transparent outside the
  circle, instead of an opaque white background, so the seal no longer hides whatever sits under
  its bounding box, including the `(인)` marker it stamps over. Seal placement now threads a
  measured shaped-glyph anchor (start, width, line height) from `hwp-render` into `hwp-convert`
  instead of a fixed per-character-advance constant: on the real test artifacts this replaces up
  to 1965 HWPUNIT of rightward drift with an exact centre-to-centre match, a geometric proxy, not
  a Hancom verdict. The product still inserts the seal image exactly as given - no white-to-alpha
  keying, no opacity warning, no new flag - and a seal overlapping the paragraphs above and below
  stays expected. Whether the seal visually sits on the `(인)` marker in genuine Hancom Office is
  the owner's own observation, collected once on this phase's final regenerated bytes, not yet at
  this commit ([#250](https://github.com/STAIxBWLB/hwp-cli/issues/250),
  [#251](https://github.com/STAIxBWLB/hwp-cli/issues/251)).
- A literal ampersand typed into an equation script now has a measured Hancom escape: wrapping it
  in ASCII double quotes, `"&"`, is what Hancom itself stores and displays as a bare `&`, measured
  against a genuine Hancom-authored ground-truth sample (12.30.0 build 6446). An unquoted, bare
  `&` remains Hancom's own reserved matrix column-alignment token and renders nothing outside a
  matrix - this project's own equation tokenizer already treated it the same way. The corrected
  escape is recorded as rule I1 in `docs/design/07-hangul-compat-rules.md` (mirrored in the
  Korean file); the tokenizer does not yet parse the quoted escape, so `hwp render`'s own raster
  of the corrected script still differs from Hancom's - an existing, already-catalogued renderer
  gap (GD-1), not a round-trip defect
  ([#252](https://github.com/STAIxBWLB/hwp-cli/issues/252)).
- Certification's `used_styles` rule no longer flags a false `style_reference_out_of_range` when
  two distinct, in-range style indices happen to share one style name, an ordinary outcome of
  `hwp merge`'s general-graft tier, which appends style tables without deduplicating by name. The
  rule now flags only a raw index with no style-table entry at all, the genuine out-of-range
  condition. Measured on the real artifact this false positive was found on: the `used_styles`
  check status moves from failed to passed, with its observed count unchanged.
- Native render's nested, non-anchored shape placement (a `Generic` control embedded inside
  another shape's own paragraph list) now adds the parent shape's own origin instead of treating
  its stored, parent-relative coordinates as page-absolute unconditionally. Measured on the real
  artifact this defect was found on: page 1's 10 `outside_page_bounds` detections drop to zero,
  every other page byte-for-byte unchanged.
...
- Release-readiness run: https://github.com/STAIxBWLB/hwp-cli/actions/runs/35482254726
- The private PDF-parity profile excludes four gates. The public one-page gate declares none
  (docs/design/21-pdf-parity.md section 4.3), so these exclusions describe the private profile
  only.
- `fonts`: the document declares its dominant body face through a `substFont`, and the oracle
